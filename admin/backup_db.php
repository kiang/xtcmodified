<?php
/**************************************************************
 * XTC Datenbank Manager Version 1.91f
...
if (RESTORE_TEST) $sim = TEXT_SIMULATION;
else $sim = '';
            ...
            if ($_SESSION['dump']['complete_inserts'] == 'yes') {
                while ($rows = mysql_fetch_array($rows_query)) {
                    ...
                    foreach ($table_list as $column) {
                        ...
                        else if ($rows[$column] != '') $insert.= '\'' . mysql_escape_string($rows[$column]) . '\',';
                        else $insert.= '\'\',';
                ...
                while ($rows = mysql_fetch_array($rows_query)) {
                    ...
                    foreach ($table_list as $column) {
                        ...
                        else if ($rows[$column] != '') $values[] = '\'' . mysql_escape_string($rows[$column]) . '\'';
                        else $values[] = '\'\'';
                        //BOF NEW TABLE  STRUCTURE  - LIKE MYSQLDUMPER
