...
class xtcImport {
    ...
    function generate_map() {
        ...
        for ($i = 0;$i < $this->catDepth;$i++) $file_layout = array_merge($file_layout, array('p_cat.' . $i => ''));
        return $file_layout;
    }
    /*****************************************************************************
     **
    ...
    function map_file($mapping) {
        if (!file_exists($this->ImportDir . $this->filename)) {
            ...
            foreach ($mapping as $key => $value) {
                ...
                foreach ($content as $key_c => $value_c) if ($key == trim($this->RemoveTextNotes($content[$key_c]))) {
    ...
    function checkImage($imgID, $pID) {
        ...
        if (!xtc_db_num_rows($img_query)) return false;
        return true;
    }
    /*****************************************************************************
     **
    ...
    function RemoveTextNotes($data) {
        if (substr($data, -1) == $this->TextSign) $data = substr($data, 1, strlen($data) - 2);
        return $data;
    }
    /*****************************************************************************
     **
    ...
    function insertProduct(&$dataArray, $mode = 'insert', $touchCat = false) {
        ...
        if ($this->FileSheme['p_tax'] == 'Y') $products_array = array_merge($products_array, array('products_tax_class_id' => $dataArray['p_tax']));
        ...
        if ($this->FileSheme['p_vpe'] == 'Y') $products_array = array_merge($products_array, array('products_vpe' => $dataArray['p_vpe']));
        if ($this->FileSheme['p_vpe_status'] == 'Y') $products_array = array_merge($products_array, array('products_vpe_status' => $dataArray['p_vpe_status']));
        if ($this->FileSheme['p_vpe_value'] == 'Y') $products_array = array_merge($products_array, array('products_vpe_value' => $dataArray['p_vpe_value']));
        ...
        if ($this->FileSheme['p_sorting'] == 'Y') $products_array = array_merge($products_array, array('products_sort' => $dataArray['p_sorting']));
        ...
        for ($i_insert = 0;$i_insert < sizeof($this->languages);$i_insert++) {
            ...
            if ($this->FileSheme['p_name.' . $this->languages[$i_insert]['code']] == 'Y') $prod_desc_array = array_merge($prod_desc_array, array('products_name' => $dataArray['p_name.' . $this->languages[$i_insert]['code']])); //web28 - 2010-08-03 - remove addslashes
            if ($this->FileSheme['p_desc.' . $this->languages[$i_insert]['code']] == 'Y') $prod_desc_array = array_merge($prod_desc_array, array('products_description' => $dataArray['p_desc.' . $this->languages[$i_insert]['code']])); //web28 - 2010-08-03 - remove addslashes
            if ($this->FileSheme['p_shortdesc.' . $this->languages[$i_insert]['code']] == 'Y') $prod_desc_array = array_merge($prod_desc_array, array('products_short_description' => $dataArray['p_shortdesc.' . $this->languages[$i_insert]['code']])); //web28 - 2010-08-03 - remove addslashes
            ...
            if ($this->FileSheme['p_keywords.' . $this->languages[$i_insert]['code']] == 'Y') $prod_desc_array = array_merge($prod_desc_array, array('products_keywords' => $dataArray['p_keywords.' . $this->languages[$i_insert]['code']]));
    ...
    function insertCategory(&$dataArray, $mode = 'insert', $pID) {
        ...
        $cat = array();
        $catTree = '';
        for ($i = 0;$i < $this->catDepth;$i++) if (trim($dataArray['p_cat.' . $i]) != '') {
...
class xtcExport {
    ...
    function exportProdFile() {
        ...
        $line = '';
        $headings = array('XTSOL', 'p_model', 'p_stock', 'p_sorting', 'p_shipping', 'p_tpl', 'p_manufacturer', 'p_fsk18', 'p_priceNoTax');
        ...
        $headings = array('p_tax', 'p_status', 'p_weight', 'p_ean', 'p_disc', 'p_opttpl', 'p_vpe', 'p_vpe_status', 'p_vpe_value');
        foreach ($headings as $heading) {
            $line.= $this->encode($heading);
        }
        // product images
