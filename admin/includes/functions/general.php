<?php
/* --------------------------------------------------------------
   $Id: general.php 1495 2010-11-13 21:24:17Z dokuman $
...
function xtc_cfg_select_option($select_array, $key_value, $key = '') {
    ...
    for ($i = 0, $n = sizeof($select_array);$i < $n;$i++) {
        ...
        if ($i == 0) $string.= '<input type="radio" name="' . $name . '" value="' . $select_array[$i] . '"';
        else $string.= '<br /><input type="radio" name="' . $name . '" value="' . $select_array[$i] . '"';
...
function get_group_price($group_id, $product_id) {
    ...
    if (!xtc_db_num_rows($group_price_query)) {
        ...
        $group_price_data = xtc_db_fetch_array($group_price_query);
    } else if (xtc_db_num_rows($group_price_query) > 1) {
