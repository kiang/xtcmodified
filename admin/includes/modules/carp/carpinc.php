...
class RSSParser {
    ...
    function FormatLink($title, $link, $class, $style, $maxtitle, $atrunc, $btitle, $atitle, $deftitle, $titles) {
        ...
        if ($didTrunc = (strlen($title) > $maxtitle)) $title = substr($title, 0, $maxtitle - strlen(preg_replace("/<.*?>/", '', $atrunc))) . $atrunc;
        if (!strlen($title)) $title = $deftitle;
        $rv = $btitle . (strlen($link = trim(str_replace('"', '&quot;', str_replace('&', '&amp;', $link)))) ? ("<a href=\"$link\"" . $this->linktargets[$carpconf['linktarget']] . ((($titles && $didTrunc) || ($titles == 2)) ? " title=\"" . str_replace('"', '&quot;', $fulltitle) . "\"" : '')) : (strlen($class . $style) ? '<span' : '')) . (strlen($class) ? (' class="' . $class . '"') : '') . (strlen($style) ? (' style="' . $style . '"') : '') . (strlen($link . $class . $style) ? '>' : '') . $title . (strlen($link) ? '</a>' : (strlen($class . $style) ? '</span>' : '')) . $atitle . "\n";
        return $rv;
    }
    ...
    function FormatDescription($description, $maxdesc, $b, $a, $atrunc) {
        ...
        if (strlen($description)) {
            if (strlen($carpconf['desctags'])) {
                $adddesc = trim(preg_replace("#<(?!" . $carpconf['desctags'] . ")(.*?)>#is", ($carpconf['removebadtags'] ? '' : "&lt;\\1\\2&gt;"), $description));
            ...
            if ($maxdesc && (strlen(preg_replace("/<.*?>/", '', $adddesc)) > $maxdesc)) {
                ...
                for ($gotchars = $i = 0, $add = '';$gotchars < $maxdesc;) {
                    ...
                    $k = 0;
                    if ((($fo = strrpos($add, '<')) > ($fc = strrpos($add, '>'))) || (($fo !== false) && ($fc === false))) $add.= substr($adddesc, $i + $j, $k = (1 + strpos(substr($adddesc, $i + $j), '>')));
                    $i+= $j + $k;
                ...
                if (!$filterblock) {
                    ...
                    for ($ioi = 0;$ioi < count($this->itemorder);$ioi++) {
                        switch ($this->itemorder[$ioi]) {
                            case "link":
                            case "title":
                                $thisitem.= $this->FormatLink($this->title, (($this->itemorder[$ioi] == 'link') ? $this->link : ''), $carpconf['ilinkclass'], $carpconf['ilinkstyle'], $carpconf['maxititle'], $carpconf['atruncititle'], $carpconf['bilink'], $carpconf['ailink'], $carpconf['defaultititle'], $carpconf['ilinktitles']);
                            break;
                            case "url":
                                $thisitem.= $this->FormatSimpleField($this->link, 'i', 'url', 1);
                            break;
                            case "desc":
                                $thisitem.= $this->FormatDescription($this->description, $carpconf['maxidesc'], $carpconf['bidesc'], $carpconf['aidesc'], $carpconf['atruncidesc']);
                            break;
                        }
                    }
                    ...
                    else $this->body.= ($this->pubdate ? $this->pubdate : (($cdate = CarpDecodeDate($this->$cdate)) ? ($cdate - $this->itemcount) : (($carpconf['lastmodified'] > 0) ? ($carpconf['lastmodified'] - $this->itemcount) : 0))) . ': :' . preg_replace("/\n/", ' ', $thisitem) . "\n";
                }
            }
            $this->insideitem = 0;
            $this->itemindex++;
    ...
    function DoEndChannel(&$data, &$order, &$b, &$a) {
        ...
        for ($coi = 0;$coi < count($order);$coi++) {
            switch ($order[$coi]) {
                case "link":
                case "title":
                    $data.= $this->FormatLink($this->ctitle, (($order[$coi] == 'link') ? $this->clink : ''), $carpconf['clinkclass'], $carpconf['clinkstyle'], $carpconf['maxctitle'], $carpconf['atruncctitle'], $carpconf['bctitle'], $carpconf['actitle'], '', $carpconf['clinktitles']);
                break;
                case "url":
                    $data.= $this->FormatSimpleField($this->clink, 'c', 'url', 1);
                break;
                case "desc":
                    $data.= $this->FormatDescription($this->cdescription, $carpconf['maxcdesc'], $carpconf['bcdesc'], $carpconf['acdesc'], $carpconf['atrunccdesc']);
                break;
            }
        }
    ...
    function characterData($parser, $data) {
        ...
        if ($this->insideitem) {
            ...
            switch ($this->tag) {
                ...
                break;
                case "DESCRIPTION":
                    $this->cdescription.= $data;
                break;
                case "LINK":
                    $this->clink.= $data;
                break;
                case "LASTBUILDDATE":
                    $this->clastbuilddate.= $data;
                break;
                case "PUBDATE":
                    $this->cpubdate.= $data;
                break;
...
function CarpDecodeDate($val) {
    global $carpconf;
    if (strlen($val)) {
        if ((($rv = strtotime($val)) == - 1) && (($rv = strtotime(preg_replace("/([0-9]+\-[0-9]+\-[0-9]+)T(.*)(?:Z|([-+][0-9]{1,2}):([0-9]{2}))/", "$1 $2 $3$4", $val))) == - 1)) $rv = 0;
    } else $rv = 0;
    return $rv + ($carpconf['timeoffset'] * 60);
}
    ...
    if (preg_match("#^http://#i", $url)) {
        ...
        if ($fp) {
            ...
            while ((!feof($fp)) && preg_match("/[^\r\n]/", $header = fgets($fp, 1000))) {
                ...
                } else if (preg_match("/^Last-Modified:/i", $header)) $carpconf['lastmodified'] = CarpDecodeDate(substr($header, 14));
            }
...
function CarpReadData($fp) {
    global $carpconf;
    return $carpconf['fixentities'] ? preg_replace("/&(?!lt|gt|amp|apos|quot)(.*\b)/is", "&amp;\\1\\2", preg_replace("/\\x00/", '', fread($fp, 4096))) : preg_replace("/\\x00/", '', fread($fp, 4096));
}
function GetRSSFeed($url, $cache, $showit) {
    ...
    if (strlen($carpconf['filterin'])) {
        ...
        for ($i = count($rss_parser->filterin) - 1;$i >= 0;$i--) {
            if (strpos($rss_parser->filterin[$i], ':') !== false) list($rss_parser->filterinfield[$i], $rss_parser->filterin[$i]) = explode(':', $rss_parser->filterin[$i], 2);
    ...
    if (strlen($carpconf['filterout'])) {
        ...
        for ($i = count($rss_parser->filterout) - 1;$i >= 0;$i--) {
            if (strpos($rss_parser->filterout[$i], ':') !== false) list($rss_parser->filteroutfield[$i], $rss_parser->filterout[$i]) = explode(':', $rss_parser->filterout[$i], 2);
