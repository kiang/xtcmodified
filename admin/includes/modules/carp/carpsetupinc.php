...
function CarpSetup0() {
    ...
    for ($i = 0;$i < $j;$i++) {
        ...
        if (($vers[$i] + 0) < $needvers[$i]) {
            echo '<span class="fail">Failed</span>. Your server is running PHP version ' . PHP_VERSION . '. CaRP requires PHP version ';
...
function CarpSetupCreateDirectories($silent = 0) {
    ...
    $rv = (file_exists($carpsetup['incdir'] . "/manualcache") || mkdir($carpsetup['incdir'] . "/manualcache", 0700)) && (file_exists($carpsetup['incdir'] . "/autocache") || mkdir($carpsetup['incdir'] . "/autocache", 0700)) && (file_exists($carpsetup['incdir'] . "/aggregatecache") || mkdir($carpsetup['incdir'] . "/aggregatecache", 0700));
    if (!$silent) {
        if ($rv) echo "Success<br />\n";
        else {
            echo '<span class="fail">Unexpected error</span><br />';
            echo 'Although the access permissions on your carp directory are correct, I am unable to create subdirectories inside it. ' . 'Unable to proceed with installation.';
        }
    }
...
function CarpSetupAccessDirectories($silent = 0) {
    ...
    if (!$silent) {
        if ($rv) echo "Success<br />\n";
        else echo '<span class="fail">Failed.</span> Unable to create files inside your cache directories. ' . 'If you created these directories manually (for example, with the command "mkdir manualcache", etc.), please delete them and run the installation script again. ' . 'If the installation script created them, then some unexected error is causing the problem.';
    }
    if ($silent) error_reporting($ser);
    return $rv;
}
...
function CarpSetup7() {
    ...
    if (preg_match('/[^0-9.]/', $ip)) {
        ...
        if ($tfp = fsockopen($ip, $port)) {
            ...
            if (strlen($carpsetup['proxyuser'])) fputs($tfp, 'Proxy-Authorization: Basic ' . base64_encode($carpsetup['proxyuser'] . ':' . $carpsetup['proxypass']) . "\r\n");
            fputs($tfp, "\r\n");
            do {
                $l = fgets($tfp, 4096);
            } while (strlen(preg_replace("/[\r\n]/", '', $l)) && !feof($tfp));
