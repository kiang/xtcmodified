<?php
/* --------------------------------------------------------------
   $Id: categories_specials.php 1398 2010-09-30 17:12:45Z web28 $
...
function showSpecialsBox() {
    ...
    if (isset($_GET['pID']) and xtc_db_num_rows($specials_query, true) > 0 and $sInfo->expires_date != 0) $expires_date = substr($sInfo->expires_date, 0, 4) . "-" . substr($sInfo->expires_date, 5, 2) . "-" . substr($sInfo->expires_date, 8, 2);
    else $expires_date = "";
    // tell the storing script if to update existing special,
    // or to insert a new one
    echo xtc_draw_hidden_field('specials_action', ((isset($_GET['pID']) and xtc_db_num_rows($specials_query, true) > 0) ? "update" : "insert"));
    ...
    if (isset($_GET['pID']) and xtc_db_num_rows($specials_query, true) > 0) echo xtc_draw_hidden_field('specials_id', $sInfo->specials_id);
?>
...
function saveSpecialsData($products_id) {
    ...
    if ($_POST['specials_action'] == "insert" and isset($_POST['specials_price']) and !empty($_POST['specials_price'])) {
        // insert a new special, code taken from /admin/specials.php, and modified
        if (!isset($_POST['specials_quantity']) or empty($_POST['specials_quantity'])) $_POST['specials_quantity'] = 0;
    ...
    } elseif ($_POST['specials_action'] == "update" and isset($_POST['specials_price']) and isset($_POST['specials_quantity'])) {
