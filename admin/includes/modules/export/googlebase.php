<?php
/* -----------------------------------------------------------------------------------------
   $Id: googlebase.php 1371 2010-09-22 09:07:14Z Tomcraft1980 $
...
class googlebase {
        ...
        if (defined('MODULE_PAYMENT_INSTALLED') && xtc_not_null(MODULE_PAYMENT_INSTALLED)) {
            ...
            for ($i = 0, $n = sizeof($installedpayments);$i < $n;$i++) {
                ...
                if (!in_array($installedpayments[$i], $unallowed_payment_modules)) {
                    if (in_array($installedpayments[$i], $creditcard_modules)) {
                        $cc = true;
                    } else if (in_array($installedpayments[$i], $americanexpress_modules)) {
                        $ae = true;
                    } else if (in_array($installedpayments[$i], $lastschrift_modules)) {
                        $la = true;
                    } else if (in_array($installedpayments[$i], $ueberweisung_modules)) {
                        $uw = true;
                    } else if (in_array($installedpayments[$i], $cash_modules)) {
                        $ca = true;
                    } else if (in_array($installedpayments[$i], $scheck_modules)) {
                        $sc = true;
                    } else {
            ...
            if ($uw == true) {
                $ueberweisung = chr(220) . 'berweisung,';
            } else {
                $ueberweisung = '';
            }
            if ($ca == true) {
            ...
            if (substr($zahlungsmethode, -1) == ',') {
                $zahlungsmethode = substr($zahlungsmethode, 0, -1);
            }
            if (substr($other_payments, -2) == ', ') {
                $other_payments = substr($other_payments, 0, -2);
            }
    ...
    function display() {
        ...
        return array('text' => EXPORT_STATUS_TYPE . '<br />' . EXPORT_STATUS . '<br />' . xtc_draw_pull_down_menu('status', $customers_statuses_array, '1') . '<br />' . CURRENCY . '<br />' . CURRENCY_DESC . '<br />' . $curr . '<b>' . MODULE_GOOGLEBASE_SHIPPING_COST_TITLE . '</b><br />' . MODULE_GOOGLEBASE_SHIPPING_COST_DESC . '<br />' . xtc_draw_input_field('shippingcosts', MODULE_GOOGLEBASE_SHIPPING_COST) . '<br />' . '<b>' . MODULE_GOOGLEBASE_SHIPPING_ART_TITLE . '</b><br />' . MODULE_GOOGLEBASE_SHIPPING_ART_DESC . '<br />' . xtc_draw_radio_field('shippingart', 'weight', true) . 'Versandksten nach Gewicht<br />' . xtc_draw_radio_field('shippingart', 'price', false) . 'Versandkosten nach Preis<br />' . SHIPPING_COUNTRY . '<br />' . SHIPPING_COUNTRY_DESC . '<br />' . xtc_draw_pull_down_menu('shipping_country', $shipping_country_array, 'DE') . '<br />' . '<b>' . MODULE_GOOGLEBASE_SUMAURL_TITLE . '</b><br />' . MODULE_GOOGLEBASE_SUMAURL_DESC . '<br />' . xtc_draw_radio_field('sumaurl', 'original', true) . 'Originale bzw. keine<br />' . xtc_draw_radio_field('sumaurl', 'shopstat', false) . 'Shopstat<br />' . xtc_draw_radio_field('sumaurl', 'directurl', false) . 'DirectURL<br />' . CAMPAIGNS . '<br />' . CAMPAIGNS_DESC . '<br />' . xtc_draw_pull_down_menu('campaign', $campaign_array) . '<br />' . EXPORT_TYPE . '<br />' . EXPORT . '<br />' . xtc_draw_radio_field('export', 'no', false) . EXPORT_NO . '<br />' . xtc_draw_radio_field('export', 'yes', true) . EXPORT_YES . '<br />' . '<br />' . xtc_button(BUTTON_EXPORT) . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=googlebase')));
    }
    function check() {
