<?php
/* -----------------------------------------------------------------------------------------
   $Id: guenstiger.php 1284 2010-09-07 23:26:23Z web28 $ 
...
class guenstiger {
    ...
    var $payment = array('MONEYORDER' => array('active' => false, 'title' => 'Vorkasse'), 'COD' => array('active' => false, 'title' => 'Nachnahme'), 'INVOICE' => array('active' => false, 'title' => 'Rechnung'), 'CC' => array('active' => false, 'title' => 'Kreditkarte'), 'BANKTRANSFER' => array('active' => false, 'title' => 'Lastschrift'), 'PAYPAL' => array('active' => false, 'title' => 'PayPal'), 'MONEYBOOKERS' => array('active' => false, 'title' => 'Moneybookers'), 'UOS_GIROPAY' => array('active' => false, 'title' => 'Giropay'));
    function guenstiger() {
        global $order;
    ...
    function setPaymentParcelServicesValues($name) {
        ...
        // take the data from the db
        $getValues = xtc_db_query("SELECT RIGHT(`configuration_key`,1)  AS `table_values` " . "FROM " . TABLE_CONFIGURATION . "  " . "WHERE `configuration_key` LIKE '%SHIPPING_" . $name . "%' " . "AND `configuration_value` LIKE '%DE%'");
        $result = xtc_db_fetch_array($getValues);
        if (isset($result['table_values']) && $result['table_values'] != '') {
        } else {
            return;
    ...
    function process($file) {
        ...
        while ($products = xtc_db_fetch_array($export_query)) {
            ...
            //create content
            $schema.= $products['products_id'] . '|' . number_format($products['products_weight'], 2, ',', '.') . '|' . $products['products_ean'] . '|' . $products['products_quantity'] . '|' . $products['manufacturers_name'] . '|' . $products['products_name'] . '|' . $products['products_model'] . '|' . number_format($products_price, 2, ',', '.') . '|' . $products_description . '|' . $products_short_description . '|' . xtc_get_shipping_status_name($products['products_shippingtime']) . '|' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG . 'product_info.php?' . $_POST['campaign'] . $gtgoogle_link . xtc_product_link($products['products_id'], $products['products_name']) . '|' . $image_if_available . '|' . substr($cat, 0, strlen($cat) - 2) . "|";
    ...
    function display() {
        ...
        return array('text' => '<br>' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG . MODULE_GUENSTIGER_EXPORT_LINK . MODULE_GUENSTIGER_FILE . '<br><br>' . MODULE_GUENSTIGER_EXPORT_LINK_SEND . MODULE_GUENSTIGER_CAMPAIGNS . '<br>' . MODULE_GUENSTIGER_CAMPAIGNS_DESC . xtc_draw_pull_down_menu('campaign', $campaign_array, 'refID=psm&') . '<br>' . MODULE_GUENSTIGER_GTGOOGLEHeader . '<br>' . MODULE_GUENSTIGER_GTGOOGLE_DESC . '<br>' . xtc_draw_radio_field('gtgoogle', 'no', !$UseGoogle) . MODULE_GUENSTIGER_GTGOOGLE_NO . '<br>' . xtc_draw_radio_field('gtgoogle', 'yes', $UseGoogle) . MODULE_GUENSTIGER_GTGOOGLE_YES . '<br><br>' . MODULE_GUENSTIGER_EXPORT_TYPE . '<br>' . MODULE_GUENSTIGER_EXPORT . '<br>' . xtc_draw_radio_field('export', 'no', true) . MODULE_GUENSTIGER_EXPORT_NO . '<br>' . xtc_draw_radio_field('export', 'yes', false) . MODULE_GUENSTIGER_EXPORT_YES . '<br><br>' . xtc_button(BUTTON_EXPORT) . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=preissuchmaschine')));
    }
    function check() {
