...
define('MODULE_NEW_STEP_IMAGE_PROCESS_TEXT_DESCRIPTION', 'Es werden nur die noch fehlenden Bilder in den Verzeichnissen<br /><br /> 

/images/product_images/popup_images/<br />

/images/product_images/info_images/<br />

/images/product_images/thumbnail_images/<br /><br /> neu erstellt.
...
');
define('MODULE_NEW_STEP_IMAGE_PROCESS_TEXT_TITLE', 'XT-Imageprocessing-New - Variante C');
...
class image_processing_new_step {
    ...
    function process($file, $offset) {
        ...
        for ($i = $offset;$i < $limit;$i++) {
            if ($i >= $max_files) xtc_redirect(xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=image_processing_new_step'));
    ...
    function display() {
        return array('text' => IMAGE_EXPORT_TYPE . '<br />' . IMAGE_EXPORT . '<br />' . '<br />' . xtc_button(BUTTON_REVIEW_APPROVE) . '&nbsp;' . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=image_processing_new_step')));
    }
    function check() {
