...
class image_processing_new_step2 {
    ...
    function process($file) {
        ...
        if ($dir = opendir(DIR_FS_CATALOG_ORIGINAL_IMAGES)) {
            while ($file = readdir($dir)) {
                if ((is_file(DIR_FS_CATALOG_ORIGINAL_IMAGES . $file)) && ($file != "index.html") && ($file != "Thumbs.db")) {
                    if ((!is_file(DIR_FS_CATALOG_THUMBNAIL_IMAGES . $file)) || (!is_file(DIR_FS_CATALOG_INFO_IMAGES . $file)) || (!is_file(DIR_FS_CATALOG_POPUP_IMAGES . $file))) {
    ...
    function display() {
        return array('text' => IMAGE_EXPORT_TYPE . '<br />' . IMAGE_EXPORT . '<br />' . '<br />' . xtc_button(BUTTON_REVIEW_APPROVE) . '&nbsp;' . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=image_processing_new_step2')));
    }
    function check() {
