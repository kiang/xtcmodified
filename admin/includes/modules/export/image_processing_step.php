...
Hierzu verarbeitet das Script nur eine begrenzte Anzahl von 10 Bildern und ruft sich danach selbst wieder auf.<br /> <br />');
...
class image_processing_step {
    ...
    function process($file, $offset) {
        ...
        for ($i = $offset;$i < $limit;$i++) {
            if ($i >= $max_files) xtc_redirect(xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=image_processing_step'));
    ...
    function display() {
        return array('text' => IMAGE_EXPORT_TYPE . '<br />' . IMAGE_EXPORT . '<br />' . '<br />' . xtc_button(BUTTON_REVIEW_APPROVE) . '&nbsp;' . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=image_processing_step')));
    }
    function check() {
