...
define('PROGRAMM_TYPE', '<hr noshade><strong>Programmart:</strong>');
...
class kelkoo {
    ...
    function process($file) {
        ...
        while ($products = xtc_db_fetch_array($export_query)) {
            ...
            //-- SNAKELAB ----//
            $schema.= $link . "\t" . $products['products_name'] . "\t" . $products_description . "\t" . number_format($products_price, 2, '.', '') . "\t" . $products['products_id'] . "\t" . $categorie_data['categories_name'] . "\t" .
            // '001'."\t".
            // 'included'. "\n";
            $kelkoo_image . "\t" . '001' . "\t" . 'EUR 4,00' . "\n";
        }
        // create File
    ...
    function display() {
        ...
        return array('text' => EXPORT_STATUS_TYPE . '<br />' . EXPORT_STATUS . '<br />' . xtc_draw_pull_down_menu('status', $customers_statuses_array, '1') . '<br />' . CURRENCY . '<br />' . CURRENCY_DESC . '<br />' . $curr . CAMPAIGNS . '<br />' . CAMPAIGNS_DESC . '<br />' . xtc_draw_pull_down_menu('campaign', $campaign_array) . '<br />' . PROGRAMM_TYPE . '<br />' . xtc_draw_radio_field('account', 'premium', false) . 'premium (400 zeichen)<br />' . xtc_draw_radio_field('account', 'normal', true) . 'normal (160 zeichen)<br />' . EXPORT_TYPE . '<br />' . EXPORT . '<br />' . xtc_draw_radio_field('export', 'no', false) . EXPORT_NO . '<br />' . xtc_draw_radio_field('export', 'yes', true) . EXPORT_YES . '<br />' . '<br />' . xtc_button(BUTTON_EXPORT) . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=kelkoo')));
    }
    function check() {
