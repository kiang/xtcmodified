...
define('MODULE_METASHOPPER_TEXT_DESCRIPTION', 'Export - Metashopper.de (; getrennt)<br /><strong>Format:</strong><br />Bestellnummer;Bezeichnung;Preis;Lieferzeit;ProduktLink;FotoLink;Beschreibung');
define('MODULE_METASHOPPER_TEXT_TITLE', 'Metashopper.de - CSV');
define('MODULE_METASHOPPER_FILE_TITLE', '<hr noshade>Dateiname');
...
class metashopper {
    ...
    function metashopper() {
        ...
        $this->title = MODULE_METASHOPPER_TEXT_TITLE;
        $this->description = MODULE_METASHOPPER_TEXT_DESCRIPTION;
        $this->sort_order = MODULE_METASHOPPER_SORT_ORDER;
        $this->enabled = ((MODULE_METASHOPPER_STATUS == 'True') ? true : false);
    }
    ...
    function display() {
        ...
        return array('text' => EXPORT_STATUS_TYPE . '<br />' . EXPORT_STATUS . '<br />' . xtc_draw_pull_down_menu('status', $customers_statuses_array, '1') . '<br />' . CURRENCY . '<br />' . CURRENCY_DESC . '<br />' . $curr . CAMPAIGNS . '<br />' . CAMPAIGNS_DESC . '<br />' . xtc_draw_pull_down_menu('campaign', $campaign_array) . '<br />' . EXPORT_TYPE . '<br />' . EXPORT . '<br />' . xtc_draw_radio_field('export', 'no', false) . EXPORT_NO . '<br />' . xtc_draw_radio_field('export', 'yes', true) . EXPORT_YES . '<br />' . '<br />' . xtc_button(BUTTON_EXPORT) . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=metashopper')));
    }
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_METASHOPPER_STATUS'");
            $this->_check = xtc_db_num_rows($check_query);
        }
        return $this->_check;
    }
    function install() {
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_METASHOPPER_FILE', 'metashopper.csv',  '6', '1', '', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_METASHOPPER_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
    function remove() {
        xtc_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('" . implode("', '", $this->keys()) . "')");
    }
    function keys() {
        return array('MODULE_METASHOPPER_STATUS', 'MODULE_METASHOPPER_FILE');
    }
}
?>