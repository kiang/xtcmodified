...
define('MODULE_PREISAUSKUNFT_TEXT_DESCRIPTION', 'Export - Preisauskunft.de (; getrennt)<br /><strong>Format:</strong><br />Bezeichnung;Artikelnr;Preis;Beschreibung;Produktlink;FotoLink;Kategorie');
define('MODULE_PREISAUSKUNFT_TEXT_TITLE', 'Preisauskunft.de - CSV');
define('MODULE_PREISAUSKUNFT_FILE_TITLE', '<hr noshade>Dateiname');
...
define('MODULE_PREISAUSKUNFT_CURRENCY_TITLE', 'W&auml;hrung');
define('MODULE_PREISAUSKUNFT_CURRENCY_DESC', 'Welche W&auml;hrung soll exportiert werden?');
...
class preisauskunft {
    ...
    function preisauskunft() {
        ...
        $this->enabled = ((MODULE_PREISAUSKUNFT_STATUS == 'True') ? true : false);
    }
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PREISAUSKUNFT_STATUS'");
            $this->_check = xtc_db_num_rows($check_query);
        }
        return $this->_check;
    }
    function install() {
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PREISAUSKUNFT_FILE', 'preisauskunft.csv',  '6', '1', '', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_PREISAUSKUNFT_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
    function remove() {
