...
pd.products_name");
        while ($products = xtc_db_fetch_array($export_query)) {
            ...
            //-- SNAKELAB ----//
            //create content
            $schema.= $products['products_id'] . '|' . $products['products_name'] . '|' . $products_description . '|' . $products_price . '|' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG_THUMBNAIL_IMAGES . $products['products_image'] . '|' . $link . '|' . '99' . '|' . (($products_price >= VERSANDKOSTENFREI_AB) ? '1' : '0') . "\n";
        }
        // create File
    ...
    function display() {
        ...
        return array('text' => EXPORT_STATUS_TYPE . '<br />' . EXPORT_STATUS . '<br />' . xtc_draw_pull_down_menu('status', $customers_statuses_array, '1') . '<br />' . CURRENCY . '<br />' . CURRENCY_DESC . '<br />' . $curr . EXPORT_TYPE . '<br />' . EXPORT . '<br />' . xtc_draw_radio_field('export', 'no', false) . EXPORT_NO . '<br />' . xtc_draw_radio_field('export', 'yes', true) . EXPORT_YES . '<br />' .
        //BOF - Tomcraft - 2009-07-21 - Exclude xtc_image_submit.inc.php as it is allready declared in /admin/includes/functions/general.php
        /*
        ...
        */
        '<br>' . xtc_button(BUTTON_EXPORT) . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=preisroboter')));
        //EOF - Tomcraft - 2009-07-21 - Exclude xtc_image_submit.inc.php as it is allready declared in /admin/includes/functions/general.php
