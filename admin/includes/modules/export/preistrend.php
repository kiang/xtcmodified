...
class preistrend {
    ...
    function process($file) {
        ...
        while ($products = xtc_db_fetch_array($export_query)) {
            ...
            $schema.= $products['products_model'] . ';' . $products['products_name'] . ';' . number_format($products_price, 2, '.', '') . ';' . xtc_get_shipping_status_name($products['products_shippingtime']) . ';' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG . 'product_info.php?' . $_POST['campaign'] . xtc_product_link($products['products_id'], $products['products_name']) . ';' . HTTP_CATALOG_SERVER . DIR_WS_CATALOG_THUMBNAIL_IMAGES . $products['products_image'] . ';' . $products_description . "\n";
        }
        // create File
