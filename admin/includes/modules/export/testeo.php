...
<strong>Modulversion</strong><br />testeo.de - <i>August 2008 - 1.2 - 29.08.2008</i><br /><br />
<strong>Fragen</strong><br />testeo publishing GmbH<br />Vorsetzen 53<br />20459 Hamburg<br /><br />Tel: 040 319 796-87<br />Fax: 040 319 796-89<br />E-Mail:<a href="mailto:haendler@testeo.de?SUBJECT=Fragen zum XT:Commerce-Modul 0.1"><u>haendler@testeo.de</u></a>');
define('MODULE_TESTEO_TEXT_TITLE', 'testeo.de - CSV');
define('MODULE_TESTEO_FILE_TITLE', '<hr noshade>Dateiname');
define('MODULE_TESTEO_FILE_DESC', 'Geben Sie einen Dateinamen ein, falls die Exportadatei am Server gespeichert werden soll.<br />(Verzeichnis export/)');
define('MODULE_TESTEO_STATUS_DESC', 'Modulstatus');
define('MODULE_TESTEO_STATUS_TITLE', 'Status');
define('MODULE_TESTEO_CURRENCY_TITLE', 'W&auml;hrung');
define('MODULE_TESTEO_CURRENCY_DESC', 'Welche W&auml;hrung soll exportiert werden?');
...
class testeo {
    ...
    function testeo() {
        global $order;
        $this->code = 'testeo';
        $this->title = MODULE_TESTEO_TEXT_TITLE;
        $this->description = MODULE_TESTEO_TEXT_DESCRIPTION;
        $this->sort_order = MODULE_TESTEO_SORT_ORDER;
        $this->CAT = array();
        $this->PARENT = array();
        $this->enabled = ((MODULE_TESTEO_STATUS == 'True') ? true : false);
    }
    ...
    function display() {
        ...
        return array('text' => EXPORT_STATUS_TYPE . '<br />' . EXPORT_STATUS . '<br />' . xtc_draw_pull_down_menu('status', $customers_statuses_array, '1') . '<br />' . CURRENCY . '<br />' . CURRENCY_DESC . '<br />' . $curr . CAMPAIGNS . '<br />' . CAMPAIGNS_DESC . '<br />' . xtc_draw_pull_down_menu('campaign', $campaign_array) . '<br />' . EXPORT_TYPE . '<br />' . EXPORT . '<br />' . xtc_draw_radio_field('export', 'no', false) . EXPORT_NO . '<br />' . xtc_draw_radio_field('export', 'yes', true) . EXPORT_YES . '<br /><br />' . xtc_button(BUTTON_EXPORT) . xtc_button_link(BUTTON_CANCEL, xtc_href_link(FILENAME_MODULE_EXPORT, 'set=' . $_GET['set'] . '&module=testeo')));
    }
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_TESTEO_STATUS'");
            $this->_check = xtc_db_num_rows($check_query);
        }
        return $this->_check;
    }
    function install() {
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_TESTEO_FILE', 'testeo.csv',  '6', '1', '', now())");
        xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_TESTEO_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
    }
    function remove() {
        xtc_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('" . implode("', '", $this->keys()) . "')");
    }
    function keys() {
        return array('MODULE_TESTEO_STATUS', 'MODULE_TESTEO_FILE');
    }
}
?>
