...
if ($action == 'shipping_edit') {
    ...
    $sql_data_array = array('orders_id' => xtc_db_prepare_input($_POST['oID']), 'title' => xtc_db_prepare_input($shipping_text), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($_POST['value']), 'class' => 'ot_shipping', 'sort_order' => xtc_db_prepare_input($shipping_order));
    //BOF - web28 - 2010-11-28 - add missing order_total_shipping_sort order
    ...
    if ((int)$check_no_tax_value['count'] > 0) {
      ...
      where orders_id = '" . $_POST['oID'] . "' 
        ...
        if ($status['customers_status_show_price_tax'] == 0 && $status['customers_status_add_tax_ot'] == 1 && MODULE_ORDER_TOTAL_SUBTOTAL_NO_TAX_STATUS) {
            ...
            $subtotal_no_tax_text = '<b>' . $xtPrice->xtcFormat($subtotal_no_tax_final, true) . '</b>';
            $sql_data_array = array('orders_id' => xtc_db_prepare_input($_POST['oID']), 'title' => MODULE_ORDER_TOTAL_SUBTOTAL_NO_TAX_TITLE, 'text' => xtc_db_prepare_input($subtotal_no_tax_text), 'value' => xtc_db_prepare_input($subtotal_no_tax_text), 'class' => 'ot_subtotal_no_tax', 'sort_order' => MODULE_ORDER_TOTAL_SUBTOTAL_NO_TAX_SORT_ORDER . ':');
            xtc_db_perform(TABLE_ORDERS_TOTAL, $sql_data_array);
        }
    }
  ...
  GROUP BY tax_rate");
    while ($ust = xtc_db_fetch_array($ust_query)) {
        ...
        if ($ust['tax_value_new']) {
            ...
            $sql_data_array = array('orders_id' => xtc_db_prepare_input($_POST['oID']), 'title' => xtc_db_prepare_input($title), 'text' => xtc_db_prepare_input($text), 'value' => xtc_db_prepare_input($ust['tax_value_new']), 'class' => 'ot_tax', 'sort_order' => MODULE_ORDER_TOTAL_TAX_SORT_ORDER);
