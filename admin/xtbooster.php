...
$xtb->config();
if (strtolower($xtb_config['MODULE_XTBOOSTER_STATUS']) != 'true') echo TXT_NOT_YET_INSTALLED;
else {
	...
	";
        ...
        if ($r['RESULT'] == 'SUCCESS') {
            ...
            #
            $auction_query = xsb_db_query("SELECT products_id, TITLE, TYPE, QUANTITY FROM xtb_auctions WHERE XTB_ITEM_ID='" . $ITEM_ID . "' LIMIT 1");
            $auction_data = xtc_db_fetch_array($auction_query);
...
?>
    ...
    elseif ($xtb_module == 'add_base') {
        ...
        if (isset($item['DESCRIPTION'])) {
            ...
            if (1 == $_SESSION['xtb1']['multi_settings']['AUTO_SUBTITLE'][0]) $item['SUBTITLE'] = $x['products_short_description'];
            ...
            foreach ($images as $k => $image) {
                ...
                if ((0 === strpos($image, 'http://')) || (0 === strpos($image, 'https://'))) {
                    ...
                    if (substr(DIR_WS_CATALOG_POPUP_IMAGES, -1) != '/' && $image[0] != '/') $images[$k] = "/" . $image;
                    if ($pi == 0) {
        ...
        foreach ($images as $pi => $image) {
            ...
            if ((0 === strpos($image, 'http://')) || (0 === strpos($image, 'https://'))) {
                ...
                if (substr(DIR_WS_CATALOG_POPUP_IMAGES, -1) != '/' && $image[0] != '/') $images[$pi - 1] = "/" . $image;
                if (ereg("src=\"*#PICTURE_" . $pi . "#\"*", $desc)) $desc = str_replace("#PICTURE_" . $pi . "#", HTTP_CATALOG_SERVER . DIR_WS_CATALOG_POPUP_IMAGES . $image, $desc);
                // BOF - Tomcraft - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                //		  elseif(ereg("#PICTURE_".$pi."#", $desc))
                elseif (preg_match("/#PICTURE_" . $pi . "#/", $desc))
                // EOF - Tomcraft - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
	...
	";
        if ($item['SUBTITLE_USE'][0] | $item['AUTO_SUBTITLE'][0]) $request.= "SUBTITLE:		-=" . base64_encode(substr(strip_tags($item['SUBTITLE']), 0, 54)) . "\n";
        $request.= "
	...
	";
        ...
        if ($item['CAT_STORE_PRIMARY'] != '') {
            ...
            if ($item['CAT_STORE_PRIMARY_DESCR'] != '') $request.= "CAT_STORE_PRIMARY_DESCR:	-=" . base64_encode($item['CAT_STORE_PRIMARY_DESCR']) . "\n";
        }
        if ($item['CAT_STORE_SECONDARY'] != '') {
            $request.= "CAT_STORE_SECONDARY:	" . $item['CAT_STORE_SECONDARY'] . "\n";
            if ($item['CAT_STORE_SECONDARY_DESCR'] != '') $request.= "CAT_STORE_SECONDARY_DESCR: -=" . base64_encode($item['CAT_STORE_PRIMARY_DESCR']) . "\n";
        }
        $request.= "SCHEDULETIME:	" . $item['SCHEDULETIME'] . "\n";
        if (@implode($item['BUYITNOW_ACTIVE']) == '1') $request.= "BUYITNOWPRICE:	" . $item['BUYITNOWPRICE'] . "\n";
        if (trim($item['SHIPPINGCOSTS']) != '') {
            ...
            if ($x[1] == '1') $request.= "SHIPTOLOCATIONS: " . $item['SHIPTOLOCATIONS'] . "\n";
            if ($item['QUANTITY'] > 1 && $item['SHIPPINGSERVICEADDITIONALCOST'] != '') $request.= "SHIPPINGSERVICEADDITIONALCOST: " . $item['SHIPPINGSERVICEADDITIONALCOST'] . "\n";
        }
        if (trim($item['SHIPPINGCOSTS1']) != '') {
            ...
            if ($x[1] == '1') $request.= "SHIPTOLOCATIONS1: " . $item['SHIPTOLOCATIONS1'] . "\n";
            if ($item['QUANTITY'] > 1 && $item['SHIPPINGSERVICEADDITIONALCOST1'] != '') $request.= "SHIPPINGSERVICEADDITIONALCOST1: " . $item['SHIPPINGSERVICEADDITIONALCOST1'] . "\n";
        }
        if (trim($item['SHIPPINGCOSTS2']) != '') {
            ...
            if ($x[1] == '1') $request.= "SHIPTOLOCATIONS2: " . $item['SHIPTOLOCATIONS2'] . "\n";
            if ($item['QUANTITY'] > 1 && $item['SHIPPINGSERVICEADDITIONALCOST2'] != '') $request.= "SHIPPINGSERVICEADDITIONALCOST2: " . $item['SHIPPINGSERVICEADDITIONALCOST2'] . "\n";
        }
        if (trim($item['SHIPPINGCOSTS3']) != '') {
            ...
            if ($x[1] == '1') $request.= "SHIPTOLOCATIONS3: " . $item['SHIPTOLOCATIONS3'] . "\n";
            if ($item['QUANTITY'] > 1 && $item['SHIPPINGSERVICEADDITIONALCOST3'] != '') $request.= "SHIPPINGSERVICEADDITIONALCOST3: " . $item['SHIPPINGSERVICEADDITIONALCOST3'] . "\n";
        }
        if (trim($item['SHIPPINGCOSTS4']) != '') {
            ...
            if ($x[1] == '1') $request.= "SHIPTOLOCATIONS4: " . $item['SHIPTOLOCATIONS4'] . "\n";
            if ($item['QUANTITY'] > 1 && $item['SHIPPINGSERVICEADDITIONALCOST4'] != '') $request.= "SHIPPINGSERVICEADDITIONALCOST4: " . $item['SHIPPINGSERVICEADDITIONALCOST4'] . "\n";
        }
        if (trim($item['SHIPPINGCOSTS5']) != '') {
            ...
            if ($x[1] == '1') $request.= "SHIPTOLOCATIONS5: " . $item['SHIPTOLOCATIONS5'] . "\n";
            if ($item['QUANTITY'] > 1 && $item['SHIPPINGSERVICEADDITIONALCOST5'] != '') $request.= "SHIPPINGSERVICEADDITIONALCOST5: " . $item['SHIPPINGSERVICEADDITIONALCOST5'] . "\n";
        }
        $res = $xtb->exec($request);
...
?>
	...
	<div class="smallText" style="font-size:arial;font-size:10px;padding:4px;background-color:#707070;color:white;border-bottom:1px solid white;">
		...
		<?php echo $request['QUANTITY'] . 'x ' . stripslashes($request['TITLE']) . ' (' . TXT_ART_NO . ' ' . $x['products_model'] . '), ' . TXT_RUNTIME . ': ' . strftime(TIME_FORMAT, $r['STARTTIME']) . ' - ' . strftime(TIME_FORMAT, $r['ENDTIME']) . ', ' . TXT_AUCTIONTYPE . ': ' . $request['TYPE'] . ', ' . TXT_STARTPRICE . ': ' . $request['STARTPRICE'];
            if ($request['BUYITNOWPRICE'] > $request['STARTPRICE']) echo ", " . TXT_BUYITNOWPRICE . ': ' . $request['BUYITNOWPRICE']; ?>
	</div>
	<?php
        } else {
            $e = unserialize($r['ERROR_MSG']);
    ...
    switch ($xtb_module) {
        case 'conf':
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                #
                if (!isset($xtb_config['MODULE_XTBOOSTER_SHOPKEY'])) xsb_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_XTBOOSTER_SHOPKEY', '" . $_POST['MODULE_XTBOOSTER_SHOPKEY'] . "', '6', '1', '', now())");
                else xsb_db_query("update " . TABLE_CONFIGURATION . " SET configuration_value = '" . $_POST['MODULE_XTBOOSTER_SHOPKEY'] . "' WHERE configuration_key = 'MODULE_XTBOOSTER_SHOPKEY'");
                # MODULE_XTBOOSTER_STDSITE
                #
                if (@$_POST['MODULE_XTBOOSTER_STDSITE'] != '') {
                    if (!isset($xtb_config['MODULE_XTBOOSTER_STDSITE'])) xsb_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_XTBOOSTER_STDSITE', '" . $_POST['MODULE_XTBOOSTER_STDSITE'] . "', '6', '1', '', now())");
                    else xsb_db_query("update " . TABLE_CONFIGURATION . " SET configuration_value = '" . $_POST['MODULE_XTBOOSTER_STDSITE'] . "' WHERE configuration_key = 'MODULE_XTBOOSTER_STDSITE'");
                }
                # MODULE_XTBOOSTER_STDCURRENCY
                #
                if (@$_POST['MODULE_XTBOOSTER_STDCURRENCY'] != '') {
                    if (!isset($xtb_config['MODULE_XTBOOSTER_STDCURRENCY'])) xsb_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_XTBOOSTER_STDCURRENCY', '" . $_POST['MODULE_XTBOOSTER_STDCURRENCY'] . "', '6', '1', '', now())");
                    else xsb_db_query("update " . TABLE_CONFIGURATION . " SET configuration_value = '" . $_POST['MODULE_XTBOOSTER_STDCURRENCY'] . "' WHERE configuration_key = 'MODULE_XTBOOSTER_STDCURRENCY'");
                }
                # MODULE_XTBOOSTER_STDSTANDORT
                #
                if (@$_POST['MODULE_XTBOOSTER_STDSTANDORT'] != '') {
                    if (!isset($xtb_config['MODULE_XTBOOSTER_STDSTANDORT'])) xsb_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_XTBOOSTER_STDSTANDORT', '" . $_POST['MODULE_XTBOOSTER_STDSTANDORT'] . "', '6', '1', '', now())");
                    else xsb_db_query("update " . TABLE_CONFIGURATION . " SET configuration_value = '" . $_POST['MODULE_XTBOOSTER_STDSTANDORT'] . "' WHERE configuration_key = 'MODULE_XTBOOSTER_STDSTANDORT'");
                }
                # MODULE_XTBOOSTER_STDPLZ
                #
                if (@$_POST['MODULE_XTBOOSTER_STDPLZ'] != '') {
                    if (!isset($xtb_config['MODULE_XTBOOSTER_STDPLZ'])) xsb_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) values ('MODULE_XTBOOSTER_STDPLZ', '" . $_POST['MODULE_XTBOOSTER_STDPLZ'] . "', '6', '1', '', now())");
                    else xsb_db_query("update " . TABLE_CONFIGURATION . " SET configuration_value = '" . $_POST['MODULE_XTBOOSTER_STDPLZ'] . "' WHERE configuration_key = 'MODULE_XTBOOSTER_STDPLZ'");
                }
...
<?php if ($extra_features['HtmlMail']): ?>
...
<script type="text/javascript">
  ...
  $('DIV_TXT_MAIL').style.display='<?php echo $txt_mail_display ?>'; 
		...
		<?php if (intval($extra_features['PicturesInHtmlMail'] != 0)) echo "(" . TXT_UP_TO . ' ' . $extra_features['PicturesInHtmlMail'] . ' ' . TXT_PICTURES . ")<br />\n";
?>
	...
	<tr class="dataTableRow">
		...
		<td class="dataTableContent">
			<select name='MODULE_XTBOOSTER_MULTIONLYONSTOCK'>
				<option<?php if ($multi_onlyonstock == 'true') {
                    echo " selected";
                } ?> value='true'><?php echo TXT_YES ?></option>
				<option<?php if ($multi_onlyonstock == 'false') {
                    echo " selected";
                } ?> value='false'><?php echo TXT_NO ?></option>
			</select>
		</td>
	...
	<tr class="dataTableRow">
		...
		<td class="dataTableContent">
			<select name='MODULE_XTBOOSTER_MULTIREVERSECATS'>
				<option<?php if ($multi_reversecats == 'true') {
                    echo " selected";
                } ?> value='true'><?php echo TXT_YES ?></option>
				<option<?php if ($multi_reversecats == 'false') {
                    echo " selected";
                } ?> value='false'><?php echo TXT_NO ?></option>
			</select>
		</td>
	...
	<tr class="dataTableRow">
		...
		<td class="dataTableContent">
			<select name='MODULE_XTBOOSTER_STDHITCOUNTER'>
				<option<?php if ($hitcounter == 'NoHitCounter') {
                    echo " selected";
                } ?> value='NoHitCounter'><?php echo TXT_NO_COUNTER ?></option>
				<option<?php if ($hitcounter == 'BasicStyle') {
                    echo " selected";
                } ?> value='BasicStyle'><?php echo TXT_STANDARD_COUNTER ?></option>
				<option<?php if ($hitcounter == 'GreenLED') {
                    echo " selected";
                } ?> value='GreenLED'><?php echo TXT_GREEN_LED_COUNTER ?></option>
				<option<?php if ($hitcounter == 'HiddenStyle') {
                    echo " selected";
                } ?> value='HiddenStyle'><?php echo TXT_HIDDEN_COUNTER ?></option>
				<option<?php if ($hitcounter == 'RetroStyle') {
                    echo " selected";
                } ?> value='RetroStyle'><?php echo TXT_RETRO_COUNTER ?></option>
			</select>
		</td>
	...
	<tr class="dataTableRow">
		...
		<td class="dataTableContent">
			<select name='MODULE_XTBOOSTER_STOCKWARNING'>
				<option<?php if ($stockwarning == 'true') {
                    echo " selected";
                } ?> value='true'><?php echo TXT_ACTIVE ?></option>
				<option<?php if ($stockwarning == 'false') {
                    echo " selected";
                } ?> value='false'><?php echo TXT_DEACTIVE ?></option>
			</select>
		</td>
	...
	<tr class="dataTableRow">
		...
		<td class="dataTableContent">
			<select name='MODULE_XTBOOSTER_REDIRECT'>
				<option<?php if ($redirect == 'basket') {
                    echo " selected";
                } ?> value='basket'><?php echo TXT_REDIRECT_BASKET ?></option>
				<option<?php if ($redirect == 'product') {
                    echo " selected";
                } ?> value='product'><?php echo TXT_REDIRECT_PRODUCT ?></option>
				<option<?php if ($redirect == 'create_account') {
                    echo " selected";
                } ?> value='create_account'><?php echo TXT_REDIRECT_CREATE_ACCOUNT ?></option>
				<option<?php if ($redirect == 'create_guest_account') {
                    echo " selected";
                } ?> value='create_guest_account'><?php echo TXT_REDIRECT_CREATE_GUEST_ACCOUNT ?></option>
			</select>
		</td>
	...
	<tr class="dataTableRow">
		...
		<td class="dataTableContent">
			<select name='MODULE_XTBOOSTER_CHANGEQUANTITY'>
				<option<?php if ($change_qtys == 'true') {
                    echo " selected";
                } ?> value='true'><?php echo TXT_YES ?></option>
				<option<?php if ($change_qtys == 'false') {
                    echo " selected";
                } ?> value='false'><?php echo TXT_NO ?></option>
			</select>
		</td>
...
?>
				<option<?php if ($default_customer_group == $d['customers_status_id']) {
                        echo " selected";
                    } ?> value='<?php echo $d['customers_status_id'] ?>'><?php echo utf8_encode($d['customers_status_name']); ?></option>
<?php
                }
?>
			</select>
...
<?php
            endif; # end of if($xtb_config['MODULE_XTBOOSTER_SHOPKEY']!=''):
             ?>
</table>
...
<?php if (!isset($_POST['add']) || @$r['RESULT'] != 'SUCCESS'): ?>

<div id='content_title' style="font-family:arial;"><?php echo TXT_EBAYPRODUCTS_ADD ?></div>

<?php if ($_POST['current_product_id'] == '' && $_GET['mode'] != 'multi_xtb'): ?>
...
<?php
                    ...
                    if ($multi_xtb) {
                        ...
                        foreach ($multi_products as $v) {
                            ...
                            $x = xtc_db_fetch_array($products_query); # This line includes GNU/GPL licensed code written by xt:Commerce GmbH (www.xtcommerce.de)
                            if ($x['products_quantity'] < 1) $any_products_quantity = $x['products_quantity'];
                            if (strlen(trim($x['products_short_description'])) > 54) $any_description_too_long = true;
                            $items[$v] = $v;
                        }
                    ...
                    if (!$multi_xtb) {
                        ...
                        if ($x['image_nr'] != '') $images[$x['image_nr']] = $x['image_name'];
                        while ($x1 = xtc_db_fetch_array($products_query)) $images[$x1['image_nr']] = $x1['image_name'];
                        if ($_SESSION['language_charset'] != 'utf-8') foreach ($x as $key => $value) $x[$key] = utf8_encode($value);
                    }
                    # Store-Info abrufen
...
<script type="text/javascript">
	...
	function toggleCurrencies(x)
	{
		...
		var spl = x.split(",");
		var y = spl[1].split("|");
		while(cur.length) cur.remove(0);
		if(y.length) for(var i=0;i<y.length;i++) cur.appendChild(new Element('option', { 'value':y[i] }).update(y[i]));
		else cur.appendChild(new Element('option', { 'value':spl[1] }).update(spl[1]));
	}
...
<?php
                    if ($stockwarning == 'true') {
                        if (!$multi_xtb && $x['products_quantity'] < 1) echo "&nbsp;&nbsp;<div class='smallText' style='font-weight:bold;color:red;font-size:13px;margin-bottom:20px;'>" . TXT_ITEM_OUT_OF_STOCK . "</div>";
                        elseif ($multi_xtb && $any_products_quantity < 1) echo "&nbsp;&nbsp;<div class='smallText' style='font-weight:bold;color:red;font-size:13px;margin-bottom:20px;'>" . TXT_ONE_OF_ITEMS_OUT_OF_STOCK . "</div>";
                    }
                    if ($multi_xtb && $any_description_too_long) echo "&nbsp;&nbsp;<div class='smallText' style='font-weight:bold;color:red;font-size:13px;margin-bottom:20px;'>" . TXT_ONE_OF_DESCRIPTIONS_TOO_LONG . "</div>";
                    if (XTBOOSTER_VERSION != '#_version#') if ($latest_version > XTBOOSTER_VERSION) echo "&nbsp;&nbsp;<div class='smallText' onclick='window.open(\"http://www.xsbooster.com/xtb/download\");' style='cursor:pointer;padding:2px;background-color:green;font-weight:bold;color:white;font-size:11px;margin-bottom:20px;'>" . TXT_NEW_XTB_VERSION_AVAILABLE . " [" . TXT_CURRENT_XTB_VERSION . ": " . $latest_version . ", " . TXT_YOUR_XTB_VERSION . ": " . XTBOOSTER_VERSION . "]</div>";
?>
...
<tr class="attributes<?php echo $rowi++ % 2 == 0 ? '-even' : '-odd'; ?>">
	...
	<td class="smallText">
		...
		<input type="checkbox" value="1" name="add[SUBTITLE_USE]" id="SUBTITLE_USE" <?php if ($products_short_description != '') {
                            echo ' checked="checked"';
                        } ?> /> <?php echo TXT_SHORTDESC_WARNING ?>
	</td>
</tr>
<?php if (strlen(strip_tags($x['products_name'])) > 54): ?>
<tr class="attributes<?php echo $rowi++ % 2 == 0 ? '-even' : '-odd'; ?>">
...
?>
		&nbsp;<?php echo TXT_OR ?>&nbsp;<select style='width:140px;' name="dummy1" onchange="$('CAT_PRIMARY').value = this.value!=''?this.value:''; onChangeCategory1(this.value);">
...
<?php
                    ...
                    if (sizeof($categories)) {
?>
		&nbsp;<?php echo TXT_OR ?>&nbsp;<select style='width:140px;' name="dummy2" onchange="$('CAT_SECONDARY').value = this.value!=''?this.value:''; onChangeCategory2(this.value);">
			<option value=""><?php echo TXT_TOP10 ?></option><option value="">--------------------</option>
<?php foreach ($categories as $categoryid => $categoryname) { ?>
			<option value="<?php echo $categoryid ?>"><?php echo $categoryname ?></option>
<?php
...
<?php
                        ...
                        foreach ($images as $image) {
                            if (substr(DIR_WS_CATALOG_POPUP_IMAGES, -1) != '/' && $image[0] != '/') $im[] = HTTP_CATALOG_SERVER . DIR_WS_CATALOG_POPUP_IMAGES . "/" . $image;
                            else $im[] = HTTP_CATALOG_SERVER . DIR_WS_CATALOG_POPUP_IMAGES . $image;
                        }
                        reset($images);
?>
...
<!-- Schauen dass absolute Adressen auch uebernommen werden
   + beruecksichtigen dass eBay https nicht nimmt-->
	<td class="smallText"><input type="text" name="add[PICTUREURL]" value="<?php if ($_POST['add']['PICTUREURL'] != '') {
                            echo $_POST['add']['PICTUREURL'];
                        } elseif ($x['products_image'] != '') {
                            if ((0 === strpos($x['products_image'], 'http://')) || (0 === strpos($x['products_image'], 'https://'))) echo str_replace('https', 'http', $x['products_image']);
                            else echo HTTP_CATALOG_SERVER . DIR_WS_CATALOG_POPUP_IMAGES . $x['products_image'];
...
<!-- Schauen dass absolute Adressen auch uebernommen werden
   + beruecksichtigen dass eBay https nicht nimmt-->
	<td class="smallText"><input type="text" name="add[GALLERY_PICTUREURL]" id="GALLERY_PICTUREURL" value="<?php if ($x['products_image'] != '') {
                            if ((0 === strpos($x['products_image'], 'http://')) || (0 === strpos($x['products_image'], 'https://'))) echo str_replace('https', 'http', $x['products_image']);
                            else echo HTTP_CATALOG_SERVER . DIR_WS_CATALOG_POPUP_IMAGES . $x['products_image'];
                        } ?>" size="50" maxlength="255"></td>
</tr>
...
<tr class="attributes<?php echo $rowi++ % 2 == 0 ? '-even' : '-odd'; ?>">
	...
	&nbsp;&nbsp;<a href="JavaScript:void(0);" onclick='$("SHP1").style.display="";' style='text-decoration:underline;'><?php echo TXT_MORE_SHIPPINGTYPES ?></a><td>
</tr>
<tr class="attributes<?php echo $rowi++ % 2 == 0 ? '-even' : '-odd'; ?>" style="display:none" id='SHP1'>
	...
	&nbsp;&nbsp;<a href="JavaScript:void(0);" onclick='$("SHP2").style.display="";' style='text-decoration:underline;'><?php echo TXT_MORE_SHIPPINGTYPES ?></a><td>
</tr>
...
<?php
            ...
            if (!isset($offset)) $offset = ($seite - 1) * $items_per_site;
            if (!isset($qu)) $qu = "";
            else $qu = addslashes(trim($qu));
            switch ($filter) {
                default:
                    # aktiv, noch ohne Verkaeufe
                    
                case 0:
                    $add = "(_EBAY_STATUS='active' AND _EBAY_QUANTITY_BUYED=0)";
                break;
                    # aktiv, erfolgreich
                ...
                case 2:
                    $add = "(_EBAY_STATUS='successful')";
                break;
                    # beendet, erfolglos
                    
                case 3:
                    $add = "(_EBAY_STATUS='unsuccessful')";
                break;
            }
            if ($qu != '') {
...
<?php
            while ($x = xtc_db_fetch_array($products_query)) # This line includes GNU/GPL licensed code written by xt:Commerce GmbH (www.xtcommerce.de)
            {
                if ($character_set_client != 'utf8') foreach ($x as $key => $value) $x[$key] = utf8_encode($value);
                switch ($x['TYPE']) {
                    case 'FixedPriceItem':
                        $x['TYPE'] = TXT_FIXPRICEAUCTION;
                    break;
                    case 'Chinese':
                        $x['TYPE'] = TXT_CHINESEAUCTION;
                    break;
                    case 'Dutch':
                        $x['TYPE'] = TXT_DUTCHAUCTION;
                    break;
                    case 'StoresFixedPrice':
                        $x['TYPE'] = TXT_STOREFIXEDPRICE;
                    break;
                }
...
?>
<tr class="attributes-<?php echo $i++ % 2 ? 'even' : 'odd'; ?>"<?php echo (($x['_EBAY_END_TIME'] < mktime() && $x['DURATION'] != 'GTC') || $x['QUANTITY'] <= $x['_EBAY_QUANTITY_BUYED']) ? ' style="color:gray;"' : ''; ?>>
...
<?php
                    while ($tx = xtc_db_fetch_array($q)) # This line includes GNU/GPL licensed code written by xt:Commerce GmbH (www.xtcommerce.de)
                    {
...
<?php
                            ...
                            $url = HTTP_SERVER . DIR_WS_CATALOG . "xtbcallback.php?item=" . $tx['HASH'] . "&key=" . $tx['XTB_KEY'];
?>
<tr>
...
<div onclick='window.open("http://developer.ebay.com/join/benefits/logo/");'
	style='cursor:pointer;width:68px;height:53px;border:0;margin:10px;background-image:url(//www.xsbooster.com/img/ebay-logo-compapp.gif);background-repeat:none;'></div>
