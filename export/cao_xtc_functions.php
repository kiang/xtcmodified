...
function SendCategories() {
    ...
    $schema = '<?xml version="1.0" encoding="' . CHARSET . '"?>' . "\n" . '<CATEGORIES>' . "\n";
    echo $schema;
    $cat_query = xtc_db_query("select categories_id, categories_image, parent_id, sort_order, date_added, last_modified " . " from " . TABLE_CATEGORIES . " order by parent_id, categories_id");
    while ($cat = xtc_db_fetch_array($cat_query)) {
        ...
        $detail_query = xtc_db_query("select categories_id, language_id,
                                  ...
                                  categories_meta_keywords, " . TABLE_LANGUAGES . ".code as lang_code, " . TABLE_LANGUAGES . ".name as lang_name from " . TABLE_CATEGORIES_DESCRIPTION . "," . TABLE_LANGUAGES . " where " . TABLE_CATEGORIES_DESCRIPTION . ".categories_id=" . $cat['categories_id'] . " and " . TABLE_LANGUAGES . ".languages_id=" . TABLE_CATEGORIES_DESCRIPTION . ".language_id");
        ...
        $prod2cat_query = xtc_db_query("select categories_id, products_id from " . TABLE_PRODUCTS_TO_CATEGORIES . " where categories_id='" . $cat['categories_id'] . "'");
...
function SendManufacturers() {
    ...
    $schema = '<?xml version="1.0" encoding="' . CHARSET . '"?>' . "\n" . '<MANUFACTURERS>' . "\n";
    echo $schema;
    $cat_query = xtc_db_query("select manufacturers_id, manufacturers_name, manufacturers_image, date_added, last_modified " . " from " . TABLE_MANUFACTURERS . " order by manufacturers_id");
    while ($cat = xtc_db_fetch_array($cat_query)) {
        ...
        $sql = "select 
             manufacturers_id, " . TABLE_MANUFACTURERS_INFO . ".languages_id, 
             manufacturers_url, 
             url_clicked, 
             date_last_click, " . TABLE_LANGUAGES . ".code as lang_code, " . TABLE_LANGUAGES . ".name as lang_name 
            from " . TABLE_MANUFACTURERS_INFO . "," . TABLE_LANGUAGES . " 
...
function SendProducts() {
    ...
    $schema = '<?xml version="1.0" encoding="' . CHARSET . '"?>' . "\n" . '<PRODUCTS>' . "\n";
    echo $schema;
    $sql = "select p.products_id,products_fsk18, products_quantity, products_model, products_image, products_price, " . "products_ean, products_date_added, products_last_modified, products_date_available, products_weight, " . "products_status, products_tax_class_id, manufacturers_id, products_ordered";
    ...
    if ((defined('TABLE_PRODUCTS_IMAGES')) and (USE_3IMAGES == true)) {
        $sql.= "left outer join " . TABLE_PRODUCTS_IMAGES . " pi1 on pi1.products_id = p.products_id and pi1.image_nr=1 " . "left outer join " . TABLE_PRODUCTS_IMAGES . " pi2 on pi2.products_id = p.products_id and pi2.image_nr=2 ";
    }
    if ((defined('TABLE_PRODUCTS_VPE')) and (USE_VPE == true)) {
    ...
    while ($products = xtc_db_fetch_array($orders_query)) {
        ...
        if (file_exists('cao_sendprod_1.php')) {
            include ('cao_sendprod_1.php');
        }
...
function SendCustomers() {
    ...
    if (xtc_db_fetch_array($res)) {
        $address_query.= "c.customers_vat_id as vat_id,";
    }
    ...
    while ($address = xtc_db_fetch_array($address_result)) {
        ...
        if (file_exists('cao_sendcust_1.php')) {
            include ('cao_sendcust_1.php');
        }
        $schema.= '</CUSTOMERS_DATA>' . "\n";
        echo $schema;
    }
...
function xtc_try_upload($file = '', $destination = '', $permissions = '777', $extensions = '') {
    ...
    if ($file_object->filename != '') return $file_object;
    else return false;
}
//--------------------------------------------------------------
...
function ManufacturersUpdate() {
    ...
    if (isset($manufacturers_id)) {
        ...
        for ($i = 0, $n = sizeof($languages);$i < $n;$i++) {
            ...
            $desc_query = xtc_db_query("select manufacturers_id,languages_id,manufacturers_url,url_clicked,date_last_click from " . TABLE_MANUFACTURERS_INFO . " where manufacturers_id='" . $manufacturers_id . "' and languages_id='" . $language_id . "'");
            ...
            if ($exists == 0) // Insert
            {
                ...
                $sql_data_array = /*xtc_*/
                array_merge($sql_data_array, $insert_sql_data);
...
function ProductsUpdate() {
    ...
    if ((defined('TABLE_PRODUCTS_VPE')) and (USE_VPE == true)) {
        if (isset($_POST['products_me'])) {
            ...
            $vpe_query = xtc_db_query("select products_vpe_id from " . TABLE_PRODUCTS_VPE . " where products_vpe_name='" . $vpe_name . "' and " . "language_id='" . $LangID . "'");
            if ($vpe_res = xtc_db_fetch_array($vpe_query)) {
                ...
                for ($i = 0, $n = sizeof($languages);$i < $n;$i++) {
                    $language_id = $languages[$i]['id'];
                    $insert_sql_data = array('products_vpe_id' => $products_vpe_id, 'language_id' => $language_id, 'products_vpe_name' => $vpe_name);
    ...
    if (file_exists('cao_produpd_1.php')) {
        include ('cao_produpd_1.php');
    }
    ...
    $sql_data_array = array('products_id' => $products_id, 'products_quantity' => $products_quantity, 'products_model' => $products_model, 'products_image' => ($products_image == 'none') ? '' : $products_image, 'products_price' => $products_price, 'products_date_available' => $products_date_available, 'products_weight' => $products_weight, 'products_status' => $products_status, 'products_ean' => $products_ean, 'products_fsk18' => $products_fsk18, 'products_shippingtime' => $products_shippingtime, 'products_tax_class_id' => $products_tax_class_id, 'manufacturers_id' => $manufacturers_id);
    if ((defined('TABLE_PRODUCTS_VPE')) and (USE_VPE == true)) {
    ...
    if ($exists == 0) // Neuanlage (ID wird an CAO zurueckgegeben !!!)
    {
        ...
        if (xtc_db_num_rows($permission_query)) {
            ...
            $insert_sql_data = array('products_date_added' => 'now()', 'products_shippingtime' => 1);
            ...
            $insert_sql_data = array('products_date_added' => 'now()', 'products_shippingtime' => 1, 'group_ids' => $group_ids);
        }
        $mode = 'APPEND';
        $sql_data_array = array_merge($sql_data_array, $insert_sql_data);
        // insert data
    ...
    for ($i = 0, $n = sizeof($languages);$i < $n;$i++) {
        ...
        $sql_data_array = array('products_name' => $products_name, 'products_description' => $products_description, 'products_short_description' => $products_short_description, 'products_meta_title' => $products_meta_title, 'products_meta_description' => $products_meta_description, 'products_meta_keywords' => $products_meta_keywords, 'products_url' => $products_url);
        if ($exists == 0) // Insert
        {
    ...
    if (defined('TABLE_PRODUCTS_IMAGES')) {
        if (isset($_POST['products_image_med'])) {
            ...
            if (strlen($_POST['products_image_med']) > 0) {
                $sql_data_array = array('products_id' => $products_id, 'image_nr' => '1', 'image_name' => xtc_db_prepare_input($_POST['products_image_med']));
        ...
        if (isset($_POST['products_image_large'])) {
            ...
            if (strlen($_POST['products_image_large']) > 0) {
                $sql_data_array = array('products_id' => $products_id, 'image_nr' => '2', 'image_name' => xtc_db_prepare_input($_POST['products_image_large']));
                xtc_db_perform(TABLE_PRODUCTS_IMAGES, $sql_data_array);
            }
        }
    }
    if (file_exists('cao_produpd_2.php')) {
        include ('cao_produpd_2.php');
    }
...
function ProductsSpecialPriceUpdate() {
    ...
    if (isset($ProdID)) {
        ...
        */
        $sp_sql = "select specials_id from " . TABLE_SPECIALS . " " . "where products_id='" . (int)$ProdID . "'";
        ...
        if ($sp = xtc_db_fetch_array($sp_query)) {
            ...
            xtc_db_query("update " . TABLE_SPECIALS . " set specials_new_products_price = '" . $Price . "'," . " specials_last_modified = now()," . " expires_date = '" . $Expire . "' where specials_id = '" . (int)$SpecialID . "'");
            print_xml_status(0, $_POST['action'], 'OK', 'UPDATE', '', '');
        } else {
            // Neuanlage
            xtc_db_query("insert into " . TABLE_SPECIALS . " (products_id, specials_new_products_price, specials_date_added, expires_date, status) " . " values ('" . (int)$ProdID . "', '" . $Price . "', now(), '" . $Expire . "', '1')");
...
function XsellUpdate() {
    ...
    if (isset($ProdID) && isset($XsellID)) {
        ...
        $SQL = "select products_id, xsell_id from " . TABLE_PRODUCTS_XSELL . " where products_id='" . $ProdID . "' and xsell_id='" . $XsellID . "'";
        $count_query = xtc_db_query($SQL);
...
function SendLog() {
    ...
    echo '<h4>Version ' . $version_nr . ' Stand : ' . $version_datum . '</h4>' . '<h4>Transfer-Log:</h4>';
    if (LOGGER == true) {
...
class upload {
    ...
    function parse() {
        ...
        if (isset($_FILES[$this->file])) {
            ...
            $file = array('name' => $GLOBALS[$this->file . '_name'], 'type' => $GLOBALS[$this->file . '_type'], 'size' => $GLOBALS[$this->file . '_size'], 'tmp_name' => $GLOBALS[$this->file]);
        }
    ...
    function save() {
        ...
        if (!function_exists(imagecreatefromgif)) {
            ...
            if ($this->destination == DIR_FS_CATALOG_ORIGINAL_IMAGES) {
                ...
                if (strstr(PRODUCT_IMAGE_THUMBNAIL_MERGE, '.gif') || strstr(PRODUCT_IMAGE_INFO_MERGE, '.gif') || strstr(PRODUCT_IMAGE_POPUP_MERGE, '.gif')) {
