<?php
/* -----------------------------------------------------------------------------------------
   $Id: google_rss_feed.php 1580 2010-12-14 14:58:39Z dokuman $
...
// cache settings
define('CACHEDAYS', 1);
define('CACHEFILE', 'export/googlebase.cache');
// cache check
...
if (GOOGLE_RSS_FEED_REFID != '' && GOOGLE_RSS_FEED_REFID != '0') $refID = '?refID=' . GOOGLE_RSS_FEED_REFID;
if (GROUP_CHECK == 'true') {
...
while ($listing = xtc_db_fetch_array($listing_query, true)) {
    ...
    if ($listing['products_description'] != '') $beschreibung = $listing['products_description'];
    elseif ($listing['products_short_description'] != '') $beschreibung = $listing['products_short_description'];
    else $beschreibung = $products_name;
    if ($listing['manufacturers_id'] > '0') $marke = xtc_db_fetch_array(xtc_db_query("SELECT manufacturers_name 
                                              FROM " . TABLE_MANUFACTURERS . "
                                              WHERE manufacturers_id = '" . $listing['manufacturers_id'] . "'"));
    // Kompatibel mit
    ...
    // EAN Nummer
    if (!empty($listing['products_ean'])) echo "\t\t<g:ean>" . $listing['products_ean'] . "</g:ean>\n";
    // Gewicht
    if ($listing['products_weight'] != '0.00') echo "\t\t<g:gewicht>" . $listing['products_weight'] . " kg</g:gewicht>\n";
    // Herstellername
    if ($listing['manufacturers_id'] > '0') echo "\t\t<g:marke>" . $marke['manufacturers_name'] . "</g:marke>\n";
    ...
    if (!empty($listing['products_model'])) echo "\t\t<g:mpn>" . $listing['products_model'] . "</g:mpn>\n";
    // Fertiger Produktpreis
    echo "\t\t<g:preis>" . $price . "</g:preis>\n";
    // Menge
    if ($listing['products_quantity'] > 0) echo "\t\t<g:menge>" . $listing['products_quantity'] . "</g:menge>\n";
    // Zustand
    // BOF - DokuMan - 2010-08-13 - set "PRODUCTS_ZUSTAND" by definition
    ...
    if (xtc_db_num_rows($cross_query)) {
        while ($xsell = xtc_db_fetch_array($cross_query, true)) {
            ...
            if (!empty($marke_komp)) echo "\t\t\t<g:marke>" . $marke_komp['manufacturers_name'] . "</g:marke>\n";
            echo "\t\t</g:kompatibel_mit>\n";
        }
    }
    // Versandkosten
...
// save cache
$fh = fopen(DIR_FS_DOCUMENT_ROOT . CACHEFILE, 'w');
fputs($fh, $content);
fclose($fh);
// show cached content
echo $content;
