 ...
 * @package Smarty
 */
/* $Id: Smarty_Compiler.class.php 1551 2010-12-04 11:23:46Z dokuman $ */
/**
...
class Smarty_Compiler extends Smarty {
    ...
    */
    ...
    function _compile_tag($template_tag) {
        /* Matched comment. */
        if (substr($template_tag, 0, 1) == '*' && substr($template_tag, -1) == '*') return '';
        ...
        switch ($tag_command) {
            ...
            case 'else':
                ...
                if ($_open_tag != 'if' && $_open_tag != 'elseif') $this->_syntax_error('unexpected {else}', E_USER_ERROR, __FILE__, __LINE__);
                else $this->_push_tag('else');
                return '<?php else: ?>';
            case 'elseif':
                list($_open_tag) = end($this->_tag_stack);
                if ($_open_tag != 'if' && $_open_tag != 'elseif') $this->_syntax_error('unexpected {elseif}', E_USER_ERROR, __FILE__, __LINE__);
                if ($_open_tag == 'if') $this->_push_tag('elseif');
            ...
            case '/section':
                ...
                if ($_open_tag == 'sectionelse') return "<?php endif; ?>";
            ...
            case '/foreach':
                ...
                if ($_open_tag == 'foreachelse') return "<?php endif; unset(\$_from); ?>";
        ...
        function _compile_block_tag($tag_command, $tag_args, $tag_modifier, &$output) {
            ...
            */
            ...
            if ($start_tag) $this->_push_tag($tag_command);
            else $this->_pop_tag($tag_command);
            if ($start_tag) {
        ...
        function _compile_section_start($tag_args) {
            ...
            foreach ($attrs as $attr_name => $attr_value) {
                ...
                if (!isset($attrs['show'])) $output.= "{$section_props}['show'] = true;\n";
            ...
            function _compile_if_tag($tag_args, $elseif = false) {
                ...
                if ($elseif) return '<?php elseif (' . implode(' ', $tokens) . '): ?>';
                else return '<?php if (' . implode(' ', $tokens) . '): ?>';
            }
            function _compile_arg_list($type, $name, $attrs, &$cache_code) {
                ...
                foreach ($attrs as $arg_name => $arg_value) {
                    if (is_bool($arg_value)) $arg_value = $arg_value ? 'true' : 'false';
                    if (is_null($arg_value)) $arg_value = 'null';
                ...
                function _parse_attrs($tag_args) {
                        ...
                        switch ($state) {
                            case 0:
                                ...
                                if (preg_match('~^\w+$~', $token)) {
                                    $attr_name = $token;
                                    $state = 1;
                                } else $this->_syntax_error("invalid attribute name: '$token'", E_USER_ERROR, __FILE__, __LINE__);
                                break;
                            case 1:
                                /* If the token is '=', then we go to state 2. */
                                if ($token == '=') {
                                    $state = 2;
                                } else $this->_syntax_error("expecting '=' after attribute name '$last_token'", E_USER_ERROR, __FILE__, __LINE__);
                                break;
                            case 2:
                                ...
                                if ($token != '=') {
                                    ...
                                    $state = 0;
                                } else $this->_syntax_error("'=' cannot be an attribute value", E_USER_ERROR, __FILE__, __LINE__);
                                break;
                            }
                            $last_token = $token;
                        }
                    ...
                    function _parse_var($var_expr) {
                        ...
                        if (count($_math_vars) > 1) {
                            ...
                            foreach ($_math_vars as $_k => $_math_var) {
                                ...
                                if (!empty($_math_var) || is_numeric($_math_var)) {
                                    ...
                                    if (preg_match('~^' . $this->_dvar_math_regexp . '$~', $_math_var)) {
                                        ...
                                        $_output.= $_math_var;
                                        if (empty($_first_var)) $_first_var = $_complete_var;
                                        $_complete_var = "";
                                    } else {
                            ...
                            if ($_has_math) {
                                if (!empty($_complete_var) || is_numeric($_complete_var)) $_output.= $this->_parse_var($_complete_var);
                                // get the modifiers working (only the last var from math + modifier is left)
                                $var_expr = $_complete_var;
                            }
                        }
                        // prevent cutting of first digit in the number (we _definitly_ got a number if the first char is a digit)
                        if (is_numeric(substr($var_expr, 0, 1))) $_var_ref = $var_expr;
                        else $_var_ref = substr($var_expr, 1);
                        if (!$_has_math) {
                            ...
                            foreach ($_indexes as $_index) {
                                if (substr($_index, 0, 1) == '[') {
                                    ...
                                    if (substr($_index, 1, 1) == '$') $_output.= "[\$this->_tpl_vars['" . substr($_index, 2) . "']]";
                                    else $_output.= "['" . substr($_index, 1) . "']";
                    ...
                    function _parse_modifiers(&$output, $modifier_string) {
                        ...
                        for ($_i = 0, $_for_max = count($_modifiers);$_i < $_for_max;$_i++) {
                            ...
                            if (count($_modifier_args) > 0) $_modifier_args = ', ' . implode(', ', $_modifier_args);
                            else $_modifier_args = '';
                            if ($_map_array) {
                    ...
                    function _compile_plugin_call($type, $name) {
                        if (isset($this->_plugins[$type][$name])) {
                            ...
                            if (is_array($this->_plugins[$type][$name][0])) {
                                return ((is_object($this->_plugins[$type][$name][0][0])) ? "\$this->_plugins['$type']['$name'][0][0]->" /* method callback */ : (string)($this->_plugins[$type][$name][0][0]) . '::' /* class callback */
