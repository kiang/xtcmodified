<?php
/* -----------------------------------------------------------------------------------------
   $Id: class.moneybookers.php 1223 2010-08-28 18:54:34Z dokuman $
...
class fcnt_moneybookers {
    ...
    function _setCode($code = 'CC', $payment_method = 'ACC') {
        ...
        if (defined('_VALID_XTC')) {
            ...
            $accepted = '';
            foreach ($icons as $key => $val) $accepted.= xtc_image('../images/icons/moneybookers/' . $val) . ' ';
    ...
    function iframeAction() {
        ...
        if (!isset($_SESSION['transaction_id'])) $_SESSION['transaction_id'] = $this->generate_trid();
        $this->insert_trid();
        ...
        $params = array('pay_to_email' => _PAYMENT_MONEYBOOKERS_EMAILID, 'transaction_id' => $_SESSION['transaction_id'], 'return_url' => xtc_href_link(FILENAME_CHECKOUT_PROCESS, 'trid=' . $_SESSION['transaction_id'], 'NONSSL', true, false), 'cancel_url' => xtc_href_link(FILENAME_CHECKOUT_PAYMENT, constant('MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_ERRORTEXT1') . $this->code . constant('MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_ERRORTEXT2'), 'SSL', true, false), 'status_url' => xtc_href_link('callback/moneybookers/callback_mb.php'), 'language' => strtoupper($_SESSION['language_code']), 'pay_from_email' => $order->customer['email_address'], 'amount' => $amount, 'currency' => $mbCurrency, 'detail1_description' => 'Shop:', 'detail1_text' => STORE_NAME . ' Order:' . $_SESSION['tmp_oID'], 'recipient_description' => STORE_NAME, 'hide_login' => '1', 'detail2_description' => 'Datum:', 'detail2_text' => strftime(DATE_FORMAT_LONG), 'amount2_description' => 'Summe:', 'amount2' => round($amount, 2), 'payment_methods' => $this->method, 'merchant_fields' => 'Field1', 'Field1' => md5(_PAYMENT_MONEYBOOKERS_MERCHANTID), 'firstname' => $order->billing['firstname'], 'lastname' => $order->billing['lastname'], 'address' => $order->billing['street_address'], 'postal_code' => $order->billing['postcode'], 'city' => $order->billing['city'], 'state' => $order->billing['state'], 'country' => $order->billing['country']['iso_code_3'], 'return_url_target' => '2', 'cancel_url_target' => '2', 'new_window_redirect' => '1', 'confirmation_note' => constant('MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_CONFIRMATION_TEXT'));
        $data = '';
    ...
    function get_error() {
        global $_GET;
        $error = array('title' => constant('MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_TEXT_ERROR'), 'error' => stripslashes(urldecode($_GET['error'])));
        return $error;
    }
    ...
    function keys() {
        return array('MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_STATUS', 'MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_SORT_ORDER', 'MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_ALLOWED', 'MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_ZONE', 'MODULE_PAYMENT_MONEYBOOKERS_' . strtoupper($this->module) . '_NEG_SHIPPING'); // Hendrik - 2010-08-11 - exlusion config for shipping modules
