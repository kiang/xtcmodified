...
class paypal_checkout {
    var $API_UserName, $API_Password, $API_Signature, $API_Endpoint, $version, $location_error, $NOTIFY_URL, $EXPRESS_CANCEL_URL, $EXPRESS_RETURN_URL, $CANCEL_URL, $RETURN_URL, $GIROPAY_SUCCESS_URL, $GIROPAY_CANCEL_URL, $BANKTXN_PENDING_URL, $EXPRESS_URL, $GIROPAY_URL, $IPN_URL, $ppAPIec, $payPalURL;
    /*************************************************************/
    function paypal_checkout() {
        ...
        if (preg_match('/^(([a-f]|[A-F]|[0-9]){6})$/', PAYPAL_API_CO_BACK)) $this->BackColor = PAYPAL_API_CO_BACK;
        //if(ereg('^(([a-f]|[A-F]|[0-9]){6})$',PAYPAL_API_CO_BORD))
        if (preg_match('/^(([a-f]|[A-F]|[0-9]){6})$/', PAYPAL_API_CO_BORD)) $this->BorderColor = PAYPAL_API_CO_BORD;
    ...
    function paypal_auth_call() {
        ...
        $nvpstr = "&AMT=" . $paymentAmount . "&CURRENCYCODE=" . $currencyCodeType . "&PAYMENTACTION=" . $paymentType . "&LOCALECODE=" . $_SESSION['language_code'] . "&RETURNURL=" . $returnURL . "&CANCELURL=" . $cancelURL . "&GIROPAYSUCCESSURL=" . $gpsucssesURL . "&GIROPAYCANCELURL=" . $gpcancelURL . "&BANKTXNPENDINGURL=" . $bankpending . "&HDRIMG=" . $this->Image . "&HDRBORDERCOLOR=" . $this->BorderColor . "&HDRBACKCOLOR=" . $this->BackColor . "&CUSTOM=" . '' . "&SHIPTONAME=" . $sh_name . "&SHIPTOSTREET=" . $sh_street . "&SHIPTOSTREET2=" . $sh_street2 . "&SHIPTOCITY=" . $sh_city . "&SHIPTOZIP=" . $sh_zip . "&SHIPTOSTATE=" . $sh_state . "&SHIPTOCOUNTRYCODE=" . $sh_countrycode . "&SHIPTOCOUNTRYNAME=" . $sh_countryname . "&PHONENUM=" . $sh_phonenum . "&ALLOWNOTE=0" . "&ADDROVERRIDE=1";
    ...
    function paypal_express_auth_call() {
        ...
        if (isset($_SESSION['sendto']) AND isset($_SESSION['customer_id'])):
            ...
            if ($_SESSION['paypal_express_new_customer'] != 'true') $address = "&SHIPTONAME=" . $sh_name . "&SHIPTOSTREET=" . $sh_street . "&SHIPTOSTREET2=" . $sh_street2 . "&SHIPTOCITY=" . $sh_city . "&SHIPTOZIP=" . $sh_zip . "&SHIPTOSTATE=" . $sh_state . "&SHIPTOCOUNTRYCODE=" . $sh_countrycode . "&SHIPTOCOUNTRYNAME=" . $sh_countryname . "&PHONENUM=" . $sh_phonenum;
        endif;
        // String zusammenbauen
    ...
    function paypal_second_auth_call($insert_id) {
        ...
        // String zusammenbauen
        $nvpstr = "&AMT=" . $paymentAmount . "&CURRENCYCODE=" . $currencyCodeType . "&PAYMENTACTION=" . $paymentType . "&NOTIFYURL=" . $notify_url . "&INVNUM=" . $inv_num . $adress . "&LOCALECODE=" . $_SESSION['language_code'] . "&RETURNURL=" . $returnURL . "&CANCELURL=" . $cancelURL . "&GIROPAYSUCCESSURL=" . $gpsucssesURL . "&GIROPAYCANCELURL=" . $gpcancelURL . "&BANKTXNPENDINGURL=" . $bankpending . "&HDRIMG=" . $this->Image . "&HDRBORDERCOLOR=" . $this->BorderColor . "&HDRBACKCOLOR=" . $this->BackColor . "&CUSTOM=" . '' . $address . "&ALLOWNOTE=0" . "&ADDROVERRIDE=1";
        // Artikel Details mitgeben
        $nvpstr.= $nvp_products;
        // Make the call to PayPal to set the Express Checkout token
        // If the API call succeded, then redirect the buyer to PayPal
    ...
    function complete_ceckout($insert_id, $data = '') {
        ...
        $address = "&SHIPTONAME=" . $sh_name . "&SHIPTOSTREET=" . $sh_street . "&SHIPTOSTREET2=" . $sh_street2 . "&SHIPTOCITY=" . $sh_city . "&SHIPTOZIP=" . $sh_zip . "&SHIPTOSTATE=" . $sh_state . "&SHIPTOCOUNTRYCODE=" . $sh_countrycode . "&SHIPTOCOUNTRYNAME=" . $sh_countryname . "&PHONENUM=" . $sh_phonenum;
        // Versand Ende
        $nvpstr = '&TOKEN=' . $token . '&PAYERID=' . $payerID . '&PAYMENTACTION=' . $paymentType . '&AMT=' . $paymentAmount . '&CURRENCYCODE=' . $currencyCodeType . '&IPADDRESS=' . $customers_ip . '&NOTIFYURL=' . $notify_url . '&INVNUM=' . $inv_num . $adress . '&BUTTONSOURCE=' . $button_source . $address;
        // Artikel Details mitgeben
        $nvpstr.= $nvp_products;
    ...
    function check_customer() {
        ...
        if (!isset($_SESSION['customer_id'])) {
            ...
            if (!xtc_db_num_rows($check_customer_query)) {
                ...
                if (PAYPAL_EXPRESS_ADDRESS_OVERRIDE == 'true' && $_SESSION['pp_allow_address_change'] != 'true') $this->create_shipping_address();
            }
        } else {
    ...
    function create_account() {
        ...
        if (PAYPAL_EXPRESS_ADDRESS_OVERRIDE == 'true'):
            if ($firstname . ' ' . $lastname != $this->UTF8decode($_SESSION['reshash']['SHIPTONAME'])) $this->create_shipping_address();
        endif;
    }
    /*************************************************************/
    function login_customer($check_customer) {
        ...
        if (SESSION_RECREATE == 'True') xtc_session_recreate();
    ...
    function create_shipping_address() {
        ...
        $tmp_country = xtc_db_fetch_array($country_query);
        $country = xtc_db_prepare_input($tmp_country['countries_id']);
        $sql_data_array = array('customers_id' => $_SESSION['customer_id'], 'entry_firstname' => $firstname, 'entry_lastname' => $lastname, 'entry_street_address' => $street_address, 'entry_postcode' => $postcode, 'entry_city' => $city, 'entry_country_id' => $country, 'entry_company' => $company, 'entry_zone_id' => '0', 'entry_state' => $state, 'address_date_added' => 'now()', 'address_last_modified' => 'now()', 'address_class' => 'paypal');
    ...
    function hash_call($methodName, $nvpStr, $pp_token = '') {
        ...
        if (function_exists('curl_init')):
            ...
            if ($this->USE_PROXY) curl_setopt($ch, CURLOPT_PROXY, PROXY_HOST . ":" . PROXY_PORT);
            ...
            */
            ...
            curl_close($ch);
            //return $nvpResArray;
            if (!$curl_fehler) return $nvpResArray;
            //else:
    ...
    function build_error_message($resArray = '', $Aufruf = '') {
        ...
        if (isset($_SESSION['curl_error_no'])) {
            ...
            while (isset($resArray["L_SHORTMESSAGE" . $count])) {
                ...
                if ($Aufruf == 'DoEx' AND ($errorCode == '10422' OR $errorCode == '10417')) $redirect = 1;
                $count = $count + 1;
                $error.= 'Error Number:' . $errorCode . '<br />';
                $error.= 'Error Short Message: ' . $shortMessage . '<br />';
                $error.= 'Error Long Message: ' . $longMessage . '<br />';
            } //end while
            if ($redirect == 1) $_SESSION['reshash']['REDIRECTREQUIRED'] = "TRUE";
        } // end else
    ...
    function paypal_get_products($paymentAmount, $order_tax, $order_discount, $order_fee, $order_shipping, $order_gs, $express_call = False) {
        ...
        for ($i = 0, $n = sizeof($order->products);$i < $n;$i++) {
            ...
            $tmp_products.= '&L_NAME' . $i . '=' . urlencode($this->mn_iconv($_SESSION['language_charset'], "UTF-8", substr($order->products[$i]['name'], 0, 127))) . '&L_NUMBER' . $i . '=' . urlencode($this->mn_iconv($_SESSION['language_charset'], "UTF-8", substr($order->products[$i]['model'], 0, 127))) . '&L_QTY' . $i . '=' . urlencode($order->products[$i]['qty']) . '&L_AMT' . $i . '=' . urlencode(number_format($products_price, $xtPrice->get_decimal_places($order->info['currency']), '.', ','));
        }
        ...
        if ($express_call AND PAYPAL_EXP_WARN != ''):
            $tmp_products.= '&L_NAME' . $i . '=' . urlencode($this->mn_iconv($_SESSION['language_charset'], "UTF-8", substr(PAYPAL_EXP_WARN, 0, 127))) . '&L_NUMBER' . $i . '=' . '&L_QTY' . $i . '=0' . '&L_AMT' . $i . '=0';
            $i++;
        endif;
        if ($express_call AND PAYPAL_EXP_VORL != '' AND PAYPAL_EXP_VERS != 0):
            $products_sum_amt+= PAYPAL_EXP_VERS;
            $tmp_products.= '&L_NAME' . $i . '=' . urlencode($this->mn_iconv($_SESSION['language_charset'], "UTF-8", substr(PAYPAL_EXP_VORL, 0, 127))) . '&L_NUMBER' . $i . '=' . '&L_QTY' . $i . '=1' . '&L_AMT' . $i . '=' . urlencode(number_format(PAYPAL_EXP_VERS, $xtPrice->get_decimal_places($order->info['currency']), '.', ','));
            $i++;
        endif;
        $products_sum_amt = round($products_sum_amt, $xtPrice->get_decimal_places($order->info['currency']));
    ...
    function write_status_history($o_id) {
        ...
        $order_history_data = array('orders_id' => $o_id, 'orders_status_id' => $o_status, 'date_added' => 'now()', 'customer_notified' => '0', 'comments' => $comment);
    ...
    function logging_status($o_id) {
        ...
        if (!$data['TRANSACTIONID'] OR $data['TRANSACTIONID'] == '') $data['TRANSACTIONID'] = 'PayPal Fehler!<br>' . date("d.m.Y - H:i:s");
        $data_array = array('xtc_order_id' => $o_id, 'txn_type' => $data['TRANSACTIONTYPE'], 'reason_code' => $data['REASONCODE'], 'payment_type' => $data['PAYMENTTYPE'], 'payment_status' => $data['PAYMENTSTATUS'], 'pending_reason' => $data['PENDINGREASON'], 'invoice' => $data['INVNUM'], 'mc_currency' => $data['CURRENCYCODE'], 'first_name' => $_SESSION['customer_first_name'], 'last_name' => $_SESSION['customer_last_name'], 'payer_business_name' => $this->UTF8decode($data['BUSINESS']), 'address_name' => $this->UTF8decode($data['SHIPTONAME']), 'address_street' => $this->UTF8decode($data['SHIPTOSTREET']), 'address_city' => $this->UTF8decode($data['SHIPTOCITY']), 'address_state' => $this->UTF8decode($data['SHIPTOSTATE']), 'address_zip' => $data['SHIPTOZIP'], 'address_country' => $this->UTF8decode($data['SHIPTOCOUNTRYNAME']), 'address_status' => $data['ADDRESSSTATUS'], 'payer_email' => $data['EMAIL'], 'payer_id' => $data['PAYERID'], 'payer_status' => $data['PAYERSTATUS'], 'payment_date' => $data['TIMESTAMP'], 'business' => '', 'receiver_email' => '', 'receiver_id' => '', 'txn_id' => $data['TRANSACTIONID'], 'parent_txn_id' => '', 'num_cart_items' => '', 'mc_gross' => $data['AMT'], 'mc_fee' => $data['FEEAMT'], 'mc_authorization' => $data['AMT'], 'payment_gross' => '', 'payment_fee' => '', 'settle_amount' => $data['SETTLEAMT'], 'settle_currency' => '', 'exchange_rate' => $data['EXCHANGERATE'], 'notify_version' => $data['VERSION'], 'verify_sign' => '', 'last_modified' => '', 'date_added' => 'now()', 'memo' => $data['DESC']);
    ...
    function callback_process($data, $charset) {
        ...
        if (isset($this->data['invoice']) && is_numeric($this->data['invoice']) && ($this->data['invoice'] > 0)) {
            ...
            if (xtc_db_num_rows($order_query) > 0) {
                ...
                if (isset($this->data['payer_status'])) $comment_status.= ' is ' . xtc_db_prepare_input($this->data['payer_status']);
                $comment_status.= '.' . $crlf;
                if (isset($this->data['test_ipn']) && is_numeric($this->data['test_ipn']) && ($_POST['test_ipn'] > 0)) $comment_status.= '(Sandbox-Test Mode)' . $crlf;
                $comment_status.= 'Total=' . xtc_db_prepare_input($this->data['mc_gross']) . xtc_db_prepare_input($this->data['mc_currency']);
                if (isset($this->data['pending_reason'])) $comment_status.= $crlf . ' Pending Reason=' . xtc_db_prepare_input($this->data['pending_reason']);
                if (isset($this->data['reason_code'])) $comment_status.= $crlf . ' Reason Code=' . xtc_db_prepare_input($this->data['reason_code']);
                ...
                if (isset($this->data['parent_txn_id'])) $comment_status.= $crlf . ' ParentID=' . xtc_db_prepare_input($this->data['parent_txn_id']);
                ...
                xtc_db_query("UPDATE " . TABLE_ORDERS . "
											...
											WHERE orders_id = '" . xtc_db_prepare_input($this->data['invoice']) . "'");
                $sql_data_array = array('orders_id' => xtc_db_prepare_input($this->data['invoice']), 'orders_status_id' => $order_status_id, 'date_added' => 'now()', 'customer_notified' => '0', 'comments' => 'PayPal IPN ' . $comment_status . '');
    ...
    function ipn_determine_txn_type($txn_type = 'unknown') {
        ...
        if ($txn_type == 'send_money') return $txn_type;
        if ($txn_type == 'express_checkout' || $txn_type == 'cart') $txn_type = $txn_type;
    ...
    function IPNdecode($string, $ipncharset = 'windows-1252', $charset) {
        // Keine Session da!
        // Stand: 29.04.2009
        if ($ipncharset != $charset) $string = $this->mn_iconv($ipncharset, $charset, $string);
        return $string;
    }
    /*************************************************************/
    function UTF8decode($string) {
        // Session vorhanden
        // Stand: 29.04.2009
        if ($this->detectUTF8($string)) $string = $this->mn_iconv('UTF-8', $_SESSION['language_charset'], $string);
        return ($string);
    }
    /*************************************************************/
