<?php
/* -----------------------------------------------------------------------------------------
   $Id: product.php 1591 2010-12-24 23:35:12Z franky-n-xtcm $
...
class product {
    ...
    function getBreadcrumbModel() {
        if ($this->data['products_model'] != "") return $this->data['products_model'];
    ...
    function getAlsoPurchased() {
        ...
        $orders_query = "select p.products_fsk18,
                            ...
                            and pd.language_id = " . (int)$_SESSION['languages_id'] . $group_check . $fsk_lock . "
    ...
    function getCrossSells() {
        ...
        if (xtc_db_num_rows($cs_groups, true) > 0) {
            while ($cross_sells = xtc_db_fetch_array($cs_groups, true)) {
                ...
                if (xtc_db_num_rows($cross_query, true) > 0) $cross_sell_data[$cross_sells['products_xsell_grp_name_id']] = array('GROUP' => xtc_get_cross_sell_name($cross_sells['products_xsell_grp_name_id']), 'PRODUCTS' => array());
    ...
    function getVPEtext($product, $price) {
        ...
        if (!is_array($product)) $product = $this->data;
    ...
    function buildDataArray(&$array, $image = 'thumbnail') {
        ...
        if ($_SESSION['customers_status']['customers_status_show_price'] != '0') {
            if ($_SESSION['customers_status']['customers_fsk18'] == '1') {
                if (isset($array['products_fsk18']) && $array['products_fsk18'] == '0') $buy_now = $this->getBuyNowButton($array['products_id'], $array['products_name']);
            } else {
    ...
    function productImage($name, $type) {
        ...
        if (empty($name)) {
            ...
            if (!file_exists($path . $name)) {
                if ($this->useStandardImage == 'true' && $this->standardImage != '') $name = $this->standardImage;
            }
