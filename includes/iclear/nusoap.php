<?php
/*
$Id: nusoap.php,v 1.2 2006/04/21 08:26:41 dis Exp $
 ...
 * @version  $Id: nusoap.php,v 1.2 2006/04/21 08:26:41 dis Exp $
 * @access   public
 */
class nusoap_base {
    /**
     ...
     * @access   public
     */
    var $typemap = array('http://www.w3.org/2001/XMLSchema' => array('string' => 'string', 'boolean' => 'boolean', 'float' => 'double', 'double' => 'double', 'decimal' => 'double', 'duration' => '', 'dateTime' => 'string', 'time' => 'string', 'date' => 'string', 'gYearMonth' => '', 'gYear' => '', 'gMonthDay' => '', 'gDay' => '', 'gMonth' => '', 'hexBinary' => 'string', 'base64Binary' => 'string',
    // abstract "any" types
    ...
    function serialize_val($val, $name = false, $type = false, $name_ns = false, $type_ns = false, $attributes = false, $use = 'encoded') {
        ...
        switch (true) {
            ...
            case (is_array($val) || $type):
                ...
                if ($valueType == 'arraySimple' || preg_match('/^ArrayOf/', $type)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                    $i = 0;
...
function timestamp_to_iso8601($timestamp, $utc = true) {
    ...
    if ($utc) {
        ...
        if (preg_match('/' . $eregStr . '/', $datestr, $regs)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
    ...
    function schemaStartElement($parser, $name, $attrs) {
        ...
        if (count($attrs) > 0) {
            foreach ($attrs as $k => $v) {
                ...
                if (preg_match("/^xmlns/", $k)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                    //$this->xdebug("$k: $v");
        ...
        switch ($name) {
            ...
            case 'complexType':
                ...
                if (isset($attrs['name'])) {
                    ...
                    //                </sequence>
                    //            </complexType>
                    if (isset($attrs['base']) && preg_match('/:Array$/', $attrs['base'])) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
...
function getTypeDef($type) {
    ...
    } elseif (preg_match('/_ContainedType$/', $type)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
...
class soap_transport_http extends nusoap_base {
    ...
    function soap_transport_http($url) {
        parent::nusoap_base();
        $this->setURL($url);
        preg_match('/\$Revisio' . 'n: ([^ ]+)/', $this->revision, $rev); // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
    ...
    */
    ...
    function getResponse() {
        ...
        if ($this->scheme == 'http' || $this->scheme == 'ssl') {
            ...
            curl_close($this->ch);
            // remove 100 header(s)
            while (preg_match('/[^HTTP/1.1 100]/', $data)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                if ($pos = strpos($data, "\r\n\r\n")) {
    ...
    */
    function parseCookie($cookie_str) {
        ...
        $data = explode(';', $cookie_str); // Hetfield - 2009-08-18 - replaced deprecated function split with explode to be ready for PHP >= 5.3
        $value_str = $data[0];
        $cookie_param = 'domain=';
...
class soap_server extends nusoap_base {
    ...
    function service($data) {
        ...
        if (preg_match('/wsdl/', $qs)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
    ...
    function parse_http_headers() {
        ...
        if (function_exists('getallheaders')) {
            ...
            if (isset($this->headers['content-type']) && strpos($this->headers['content-type'], '=')) {
                ...
                if (preg_match('/^(ISO-8859-1|US-ASCII|UTF-8)$/i', $enc)) { // Hetfield - 2009-08-19 - replaced deprecated function eregi with preg_match to be ready for PHP >= 5.3
...
class wsdl extends nusoap_base {
    ...
    function start_element($parser, $name, $attrs) {
        if ($this->status == 'schema') {
        ...
        } elseif (preg_match('/schema$/', $name)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
            ...
            if (preg_match('/:/', $name)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                // get ns prefix
    ...
    function end_element($parser, $name) {
        ...
        preg_match('/schema$/', $name)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
            $this->status = "";
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
    ...
    function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = 'rpc', $use = 'encoded', $documentation = '', $encodingStyle = '') {
        ...
        // get binding
        $this->bindings[$this->serviceName . 'Binding']['operations'][$name] = array('name' => $name, 'binding' => $this->serviceName . 'Binding', 'endpoint' => $this->endpoint, 'soapAction' => $soapaction, 'style' => $style, 'input' => array('use' => $use, 'namespace' => $namespace, 'encodingStyle' => $encodingStyle, 'message' => $name . 'Request', 'parts' => $in), 'output' => array('use' => $use, 'namespace' => $namespace, 'encodingStyle' => $encodingStyle, 'message' => $name . 'Response', 'parts' => $out), 'namespace' => $namespace, 'transport' => 'http://schemas.xmlsoap.org/soap/http', 'documentation' => $documentation);
        // add portTypes
        // add messages
        if ($in) {
...
class soap_parser extends nusoap_base {
    ...
    function start_element($parser, $name, $attrs) {
        ...
        foreach ($attrs as $key => $value) {
            ...
            // if ns declarations, add to class level array of valid namespaces
            if ($key_prefix == 'xmlns') {
                if (preg_match('/^[http://www.w3.org/][0-9]{4}[/XMLSchema]$/', $value)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                ...
                */
                ...
                if (preg_match('/' . $expr . '/', $value, $regs)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
    ...
    */
    ...
    function send($msg, $soapaction = '', $timeout = 0, $response_timeout = 30) {
        ...
        switch (true) {
            ...
            case preg_match('/^http/', $this->endpoint): // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                ...
                if (preg_match('/^http:/', $this->endpoint)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
                    //if(strpos($this->endpoint,'http:')){
                    $this->responseData = $http->send($msg, $timeout, $response_timeout, $this->cookies);
                } elseif (preg_match('/^https/', $this->endpoint)) { // Hetfield - 2009-08-19 - replaced deprecated function ereg with preg_match to be ready for PHP >= 5.3
