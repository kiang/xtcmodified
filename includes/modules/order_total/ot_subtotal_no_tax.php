...
class ot_subtotal_no_tax {
    ...
    function process() {
        ...
        if ($_SESSION['customers_status']['customers_status_show_price_tax'] == 0 && $_SESSION['customers_status']['customers_status_add_tax_ot'] == 1) {
            ...
            $this->output[] = array('title' => $this->title . ':', 'text' => '<strong>' . $xtPrice->xtcFormat($sub_total_price + ($xtPrice->xtcFormat($order->info['shipping_cost'], false, 0, true)), true) . '</strong>', 'value' => $xtPrice->xtcFormat($sub_total_price + ($xtPrice->xtcFormat($order->info['shipping_cost'], false, 0, true)), false));
        }
    }
    function check() {
