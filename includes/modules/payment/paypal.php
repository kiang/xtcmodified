...
class paypal {
    ...
    function install() {
        ...
        foreach ($stati as $statusname => $statusid) {
            ...
            while ($languages = xtc_db_fetch_array($languages_query)) {
                if (file_exists(DIR_FS_LANGUAGES . $languages['directory'] . '/admin/paypal.php')) include (DIR_FS_LANGUAGES . $languages['directory'] . '/admin/paypal.php');
                if ($$statusname != ''):
        ...
        foreach ($new_config as $v1) {
            ...
            xtc_db_query("insert into " . TABLE_CONFIGURATION . $m_fields . "values (" . "'" . (($old_config) ? $old_config[1] : $v1[1]) . "', " . "'" . (($old_config) ? $old_config[2] : $v1[2]) . "', " . (($old_config) ? $old_config[3] : $v1[3]) . ", " . $v1[4] . ", NULL, now(), '" . $v1[5] . "', '" . $v1[6] . "' )");
        }
        ...
        if (xtc_db_num_rows($check_query) < 1):
            ...
            if ($_SESSION['customer_id'] != 1) xtc_db_query("update " . TABLE_ADMIN_ACCESS . " SET paypal=1 where customers_id=" . $_SESSION['customer_id'] . " LIMIT 1");
        endif;
        ...
        if (xtc_db_num_rows($check_query) < 1):
            ...
            if ($_SESSION['customer_id'] != 1) xtc_db_query("update " . TABLE_ADMIN_ACCESS . " SET module_paypal_install=1 where customers_id=" . $_SESSION['customer_id'] . " LIMIT 1");
        endif;
        // Table paypal
    ...
    function remove($pre_inst = 0) {
        ...
        if (!$_POST['paypaldelete'] AND !$pre_inst):
            ...
            if (xtc_db_num_rows($check_query) == 0) xtc_redirect(xtc_href_link(FILENAME_MODULES, 'set=' . $_GET['set'] . '&module=' . $this->code . '&action=removepaypal'));
        endif;
    ...
    function keys() {
        // Stand: 29.04.2009
        return array('MODULE_PAYMENT_PAYPAL_STATUS', 'MODULE_PAYMENT_PAYPAL_ALLOWED', 'MODULE_PAYMENT_PAYPAL_ZONE', 'MODULE_PAYMENT_PAYPAL_SORT_ORDER', 'MODULE_PAYMENT_PAYPAL_NEG_SHIPPING'); // Hendrik - 2010-08-11 - exlusion config for shipping modules
