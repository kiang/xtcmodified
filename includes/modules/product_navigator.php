<?php
/* -----------------------------------------------------------------------------------------
   $Id: product_navigator.php 1304 2010-09-14 13:48:41Z gtb-modified $
...
if ($actual_key == 0) {
    ...
    if ($prev_id != 0) $first_link = xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($p_data[0]['pID'], $p_data[0]['pName']));
}
// check if key = last
if ($actual_key == (sizeof($p_data) - 1)) {
    ...
    if ($next_id != (sizeof($p_data) - 1)) $last_link = xtc_href_link(FILENAME_PRODUCT_INFO, xtc_product_link($p_data[(sizeof($p_data) - 1) ]['pID'], $p_data[(sizeof($p_data) - 1) ]['pName']));
}
