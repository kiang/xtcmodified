<?php
/* -----------------------------------------------------------------------------------------
   $Id: gls.php 1508 2010-11-20 20:16:09Z dokuman $
...
class gls {
    ...
    function quote($method = '') {
        ...
        if ($dest_plz_in) {
            $this->quotes = array('id' => $this->code, 'module' => MODULE_SHIPPING_GLS_TEXT_TITLE, 'methods' => array(array('id' => $this->code, 'title' => $shipping_method . ' (' . $shipping_num_boxes . ' x ' . $shipping_weight . ' ' . MODULE_SHIPPING_GLS_TEXT_UNITS . ' = ' . $shipping_cost_normal_formatted . ' ' . $tax_text . ')' . ' ' . MODULE_SHIPPING_GLS_POSTCODE_INFO_TEXT . ': (' . $shipping_cost_extra_formatted . ' ' . $tax_text . ')', 'cost' => $shipping_cost * $shipping_num_boxes)));
        } else {
            $this->quotes = array('id' => $this->code, 'module' => MODULE_SHIPPING_GLS_TEXT_TITLE, 'methods' => array(array('id' => $this->code, 'title' => $shipping_method . ' (' . $shipping_num_boxes . ' x ' . $shipping_weight . ' ' . MODULE_SHIPPING_GLS_TEXT_UNITS . ')', 'cost' => $shipping_cost * $shipping_num_boxes)));
        }
        if ($this->tax_class > 0) {
