...
class hermes {
    ...
    function hermes() {
        ...
        if ($this->enabled == true && count($order->products) > 0) {
            ...
            if ($gew <= MODULE_SHIPPING_HERMES_MAXGEWICHT) $check_flag = true;
            if ($check_flag == false) {
                $this->enabled = false;
            }
        }
    ...
    function quote($method = '') {
        ...
        if ($order->delivery['country']['iso_code_2'] == 'DE') $preise = preg_split("/;/", MODULE_SHIPPING_HERMES_NATIONAL); // Hetfield - 2009-11-19 - replaced deprecated function split with preg_split to be ready for PHP >= 5.3
        else $preise = preg_split("/;/", MODULE_SHIPPING_HERMES_INTERNATIONAL); // Hetfield - 2009-11-19 - replaced deprecated function split with preg_split to be ready for PHP >= 5.3
        ...
        if ($order->delivery['country']['iso_code_2'] == 'DE') $stitle = MODULE_SHIPPING_HERMES_TEXT_WAY_DE . $shipping_weight . ' ' . MODULE_SHIPPING_HERMES_TEXT_UNITS;
        else $stitle = MODULE_SHIPPING_HERMES_TEXT_WAY_EU . $shipping_weight . ' ' . MODULE_SHIPPING_HERMES_TEXT_UNITS;
