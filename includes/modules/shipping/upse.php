...
class upse {
    ...
    function upse() {
        ...
        if (($this->enabled == true) && ((int)MODULE_SHIPPING_UPSE_ZONE > 0)) {
            $check_flag = false;
            $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_UPSE_ZONE . "' and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
    ...
    function quote($method = '') {
        ...
        $this->quotes = array('id' => $this->code, 'module' => MODULE_SHIPPING_UPSE_TEXT_TITLE, 'methods' => array(array('id' => $this->code, 'title' => $shipping_method . ' (' . $shipping_num_boxes . ' x ' . $shipping_weight . ' ' . MODULE_SHIPPING_UPSE_TEXT_UNITS . ')', 'cost' => $shipping_cost * $shipping_num_boxes)));
        if ($this->tax_class > 0) {
