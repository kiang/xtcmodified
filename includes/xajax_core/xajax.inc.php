 ...
 *
 * @package xajax
 * @version $Id: xajax.inc.php 1194 2010-08-22 07:29:37Z hen-vm68 $
...
class xajax {
    ...
    */
    ...
    function xajax($sRequestURI = "", $sWrapperPrefix = "xajax_", $sEncoding = XAJAX_DEFAULT_CHAR_ENCODING, $bDebug = false) {
        ...
        if ($this->sRequestURI == "") $this->sRequestURI = $this->_detectURI();
    ...
    function getRequestMode() {
        if (!empty($_GET["xajax"])) return XAJAX_GET;
        if (!empty($_POST["xajax"])) return XAJAX_POST;
        return -1;
    }
    ...
    function processRequests() {
        ...
        if ($requestMode == XAJAX_POST) {
            ...
            if (!empty($_POST["xajaxargs"])) $aArgs = $_POST["xajaxargs"];
        } else {
            ...
            if (!empty($_GET["xajaxargs"])) $aArgs = $_GET["xajaxargs"];
        }
        ...
        if ($this->sEncoding && strlen(trim($this->sEncoding)) > 0) $sContentHeader.= " charset=" . $this->sEncoding;
        ...
        if ($rootTag == "xjxquery") {
            ...
            if (get_magic_quotes_gpc() == 1) {
                ...
                foreach ($aArray as $sKey => $sValue) {
                    if (is_string($sValue)) $newArray[$sKey] = stripslashes($sValue);
                    else $newArray[$sKey] = $sValue;
                }
                $aArray = $newArray;
            }
        }
