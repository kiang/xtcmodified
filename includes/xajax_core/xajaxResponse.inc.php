 ...
 *
 * @package xajax
 * @version $Id: xajaxResponse.inc.php 1194 2010-08-22 07:29:37Z hen-vm68 $
    ...
    */
    ...
    function addRedirect($sURL, $iDelay = 0) {
        ...
        if ($queryStart !== FALSE) {
            ...
            if ($queryEnd === FALSE) $queryEnd = strlen($sURL);
            ...
            if ($queryParts) {
                ...
                foreach ($queryParts as $key => $value) {
                    ...
                    else $newQueryPart.= ini_get('arg_separator.output');
        ...
        if ($iDelay) $this->addScript('window.setTimeout("window.location = \'' . $sURL . '\';",' . ($iDelay * 1000) . ');');
        else $this->addScript('window.location = "' . $sURL . '";');
        return $this;
    }
    ...
    function loadXML($mCommands) {
        if (is_a($mCommands, 'xajaxResponse')) {
            ...
            if (!empty($mCommands)) trigger_error("The xajax response output could not load other commands as data was not a valid array", E_USER_ERROR);
        }
    }
    ...
    function _cmdXML($aAttributes, $sData) {
        ...
        $xml = "<cmd";
        foreach ($aAttributes as $sAttribute => $sValue) $xml.= " $sAttribute=\"$sValue\"";
        if ($sData !== null && !stristr($sData, '<![CDATA[')) $xml.= "><![CDATA[$sData]]></cmd>";
        else if ($sData !== null) $xml.= ">$sData</cmd>";
        else $xml.= "></cmd>";
        return $xml;
    }
    /**
    ...
    function _getXMLForCommand($aAttributes, $mData) {
        $xml = '<cmd';
        foreach ($aAttributes as $sAttribute => $sValue) if ($sAttribute) $xml.= " $sAttribute=\"$sValue\"";
        if (is_array($mData)) $xml.= '>' . $this->_arrayToXML($mData) . '</cmd>';
        else $xml.= '>' . $this->_escape($mData) . '</cmd>';
        return $xml;
    }
    /**
    ...
    function _arrayToXML($mArray) {
        if (!is_array($mArray)) return $this->_escape($mArray);
        $xml = '<xjxobj>';
    ...
    function _escape($sData) {
        if (!is_numeric($sData) && !$sData) return '';
        $needCDATA = false;
        if ($this->bOutputEntities) {
            if (!function_exists('mb_convert_encoding')) trigger_error('The xajax response output could not be converted to HTML entities because the mb_convert_encoding function is not available', E_USER_NOTICE);
            $sData = call_user_func_array('mb_convert_encoding', array(&$sData, 'HTML-ENTITIES', $this->sEncoding));
        } else {
            if ((strpos($sData, '<![CDATA[') !== false) || (strpos($sData, ']]>') !== false) || (htmlentities($sData) != $sData)) $needCDATA = true;
            ...
            foreach ($segments as $key => $segment) {
                ...
                foreach ($fragments as $fragment) {
                    if ($segment != '') $segment.= ']]]]><![CDATA[>';
                    $segment.= $fragment;
                }
                if ($sData != '') $sData.= '<![]]><![CDATA[CDATA[';
                $sData.= $segment;
            }
        }
        if ($needCDATA) $sData = '<![CDATA[' . $sData . ']]>';
        return $sData;
    }
    /**
    ...
    function _buildObj($mData) {
        ...
        if (is_array($mData)) {
            $aData = array();
            foreach ($mData as $key => $value) $aData[] = array('k' => $this->_buildObj($key), 'v' => $this->_buildObj($value));
            return $aData;
        } else return $mData;
    }
