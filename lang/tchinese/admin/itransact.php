<?php
/* --------------------------------------------------------------
   $Id: itransact.php 36 2006-12-28 13:19:07Z sun $

   xt:Commerce - community made shopping
   http://www.xt-commerce.com
   http://www.xt-commerce.cn

   Copyright (c) 2007 xt:Commerce Shopsoftware
   --------------------------------------------------------------
   based on: 
   (c) 2000-2001 The Exchange Project  (earlier name of osCommerce)
   (c) 2002-2003 osCommerce(itransact.php,v 1.4 2002/03/30); www.oscommerce.com 
   (c) 2003	 nextcommerce (itransact.php,v 1.4 2003/08/14); www.nextcommerce.org

   Released under the GNU General Public License 
   -------------------------------------------------------------- */

define('HEADING_TITLE', '開啟CPANEL虛擬主機空間管理平台');
define('SUB_BAR_TITLE', '點下列圖片，你必須要有ID 和密碼');

define('CPANEL_URL', 'https://yoursite:cpanelport');
define('CPANEL_URL_NAME', 'CPANEL管理平台名稱');
?>