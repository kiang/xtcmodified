...
#-- Deny direct access
if (preg_match("/" . basename(__FILE__) . "/", $_SERVER['SCRIPT_NAME']) || preg_match("/" . basename(__FILE__) . "/", $_SERVER['PHP_SELF'])) {
    if ($shopstat_debug) {
        $shopstat_debug_msg.= '\nRESULT:\n------------\nNOTLOGGED->DIRECT ACCESS!';
        print "<script>alert('" . $shopstat_debug_msg . "');</script>";
    }
    return;
}
#-- Allready logged, so we return
if (isset($shoplog_islogged) && $shoplog_islogged) {
    if ($shopstat_debug) {
        $shopstat_debug_msg.= '\nRESULT:\n------------\nNOTLOGGED->ALLREADY LOGGED!';
        print "<script>alert('" . $shopstat_debug_msg . "');</script>";
    }
    return;
}
...
if ($shopstat_debug) {
    ...
    foreach ($_REQUEST as $key => $value) {
        $shopstat_debug_msg.= $key . '=' . $value . '\n';
    }
    print '<script>alert(\'' . $shopstat_debug_msg . '\');</script>';
}
$shoplog_prodid = "";
if (isset($_GET['products_id']) || isset($_GET['BUYproducts_id'])) {
    switch ($log_action) {
        case 'add_product':
            $shoplog_method = 'add';
        break;
    ...
    if (preg_match("/checkout_shipping\.php/", $_SERVER['SCRIPT_NAME']) || preg_match("/checkout_shipping\.php/", $_SERVER['PHP_SELF'])) {
        $shoplog_method = 'buy1';
    } elseif (preg_match("/checkout_payment\.php/", $_SERVER['SCRIPT_NAME']) || preg_match("/checkout_payment\.php/", $_SERVER['PHP_SELF'])) {
        $shoplog_method = 'buy2';
    } elseif (preg_match("/checkout_confirmation\.php/", $_SERVER['SCRIPT_NAME']) || preg_match("/checkout_confirmation\.php/", $_SERVER['PHP_SELF'])) {
        $shoplog_method = 'buy3';
    } elseif (preg_match("/checkout_process\.php/", $_SERVER['SCRIPT_NAME']) || preg_match("/checkout_process\.php/", $_SERVER['PHP_SELF'])) {
        $shoplog_method = 'validate_and_send';
    } elseif (isset($_GET['keywords']) && (preg_match("/search/", $_SERVER['SCRIPT_NAME']) || preg_match("/search/", $_SERVER['PHP_SELF']))) {
    ...
    } elseif (isset($_SERVER['SCRIPT_NAME']) && (preg_match("/shopping_cart\.php/", $_SERVER['SCRIPT_NAME']) || preg_match("/shopping_cart\.php/", $_SERVER['PHP_SELF']))) {
        $shoplog_method = 'review';
    } elseif (isset($_GET['cPath']) || (preg_match("/index\.php/", $_SERVER['SCRIPT_NAME']) || preg_match("/index\.php/", $_SERVER['PHP_SELF']))) {
...
if ((!(isset($shoplog_mode) && $shoplog_mode)) && ((isset($shoplog_method) && $shoplog_method == 'buy1') || (isset($shoplog_method) && $shoplog_method == 'buy2') || (isset($shoplog_method) && $shoplog_method == 'buy3') || (isset($shoplog_method) && $shoplog_method == 'listitems') || (isset($shoplog_method) && $shoplog_method == 'showitem') || (isset($shoplog_method) && $shoplog_method == 'search'))) {
    if ($shopstat_debug) {
        $shopstat_debug_msg.= '\nMETHOD:\n------------\n' . $shoplog_method;
        $shopstat_debug_msg.= '\nRESULT:\n------------\nNOTLOGGED!';
        print "<script>alert('" . $shopstat_debug_msg . "');</script>";
...
(!empty($shoplog_products['products_model'])) ? $shopstat_prodartnr = $shoplog_products['products_model'] : $shopstat_prodartnr = $shoplog_prodid;
//------------------------------------------------------------------------------
...
if ((isset($shoplog_method) && $shoplog_method == 'add') || (isset($shoplog_method) && $shoplog_method == 'delitem')) {
    #-- article# & articlename
...
elseif (isset($shoplog_method) && $shoplog_method == 'showitem') {
    ...
    //urlencode(strip_tags(xtc_get_products_name($shoplog_prodid,$languages_id))).'&amp;'.
    //BOF - DokuMan - 2010-10-13 - languages_id in xtc_get_products_name can be empty, but returns nasty php notice
    urlencode(strip_tags(xtc_get_products_name($shoplog_prodid, $_SESSION['languages_id']))) . '&amp;' . urlencode(strip_tags($shoplog_cat));
    //EOF - DokuMan - 2010-10-13 - languages_id in xtc_get_products_name can be empty, but returns nasty php notice
    
}
#-- user browse through the products
elseif (isset($shoplog_method) && ($shoplog_method == 'listitems')) {
    (isset($shoplog_mode) && $shoplog_mode) ? $shoplog_query = urlencode(strip_tags($shoplog_cat)) : false;
}
...
elseif (isset($shoplog_method) && $shoplog_method == 'validate_and_send') {
    ...
    if (is_object($order)) {
        ...
        $shoplog_query = ((isset($order->info['total'])) ? $order->info['total'] : '') . '&amp;' . ((isset($order->info['payment_method'])) ? $order->info['payment_method'] : '') . '&amp;' . ((isset($order->info['shipping_cost'])) ? $order->info['shipping_cost'] : '') . '->';
        #-- Ordered products
        if (isset($order->products)) {
            ...
            foreach ($order->products as $item) {
                ...
                $shoplog_query.= implode('&amp;', array($item['model'], urlencode(strip_tags($item['name'])), urlencode(strip_tags(shopstat_getRealPath(xtc_get_product_path(xtc_get_prid($item['id']))))), $item['qty'], $item['price']));
                $init = 0;
            }
        }
    }
...
if ($logit && !(isset($shoplog_islogged) && $shoplog_islogged)) {
    ...
    if (!@file_exists($sl_logfilename)) {
        ...
        (function_exists("set_file_buffer")) ? @set_file_buffer($sl_handle, 0) : false;
        @flock($sl_handle, 2);
        ...
        while (true) {
            ...
            */
            ...
            if (file_exists($sl_logfilename_last)) { # pack theold file ...
    ...
    (function_exists("set_file_buffer")) ? @set_file_buffer($sl_handle, 0) : false;
    @flock($sl_handle, 2);
    $sl_log = implode("\t", array($sl_datestr, $sl_timestr, $shoplog_ip, str_replace(" ", "+", $shoplog_useragent), $shoplog_referer, urlencode($shoplog_id), $shoplog_method, urlencode($shoplog_cat), $shoplog_query)) . "\n";
