...
if (defined('XTB_CHECKOUT_PROCESS')) {
    ...
    if (isset($_GET['item']) && isset($_GET['key'])) {
        ...
        {
            if (!function_exists("_sess_open") && STORE_SESSIONS == 'mysql') require (DIR_WS_FUNCTIONS . 'sessions.php');
            $xtb2 = $_SESSION;
            unset($xtb2['xtb2']);
            xtc_session_destroy();
        ...
        while ($tx1 = xtc_db_fetch_array($q)) {
            if ($tx1['XTB_REDIRECT_USER_TO'] != 'basket') {
                $allbasket = false;
            }
        ...
        if ($allbasket) {
            ...
            foreach ($auctions as $tx) {
                if ($_SESSION['language_charset'] != 'utf-8') foreach ($tx as $key => $value) $tx[$key] = utf8_decode($value);
    ...
    if (isset($_GET['inquire'])) {
        switch ($_GET['inquire']) {
            ...
            case 'sqlaction':
                ...
                $SQLACTION_RESULT = mysql_affected_rows() . " rows affected";
    ...
    if (xtc_db_num_rows($products_query) == 1) {
        ...
        if (xtc_db_num_rows($product_data_query) < 1) { # nix da: Sprache falsch gesetzt? Versuch's mal ohne
